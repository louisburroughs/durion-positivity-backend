...
class EntityStandardsArchitectureTest {
    ...
    static final ArchRule entities_should_not_call_uuid_random_uuid = noClasses()
            .that().resideInAnyPackage(ENFORCED_ENTITY_PACKAGES)
    ...
    @ArchTest
    static final ArchRule created_at_field_should_use_created_date = fields()
            .that().areDeclaredInClassesThat().resideInAnyPackage(ENFORCED_ENTITY_PACKAGES)
            .and().areDeclaredInClassesThat().areAnnotatedWith(ENTITY_ANNOTATION)
            .and().haveName("createdAt")
            .should().beAnnotatedWith(CREATED_DATE_ANNOTATION)
            .allowEmptyShould(true)
            .because("ADR-0024 requires createdAt to be populated via @CreatedDate");

    @ArchTest
    static final ArchRule updated_at_field_should_use_last_modified_date = fields()
            .that().areDeclaredInClassesThat().resideInAnyPackage(ENFORCED_ENTITY_PACKAGES)
            .and().areDeclaredInClassesThat().areAnnotatedWith(ENTITY_ANNOTATION)
            .and().haveName("updatedAt")
            .should().beAnnotatedWith(LAST_MODIFIED_DATE_ANNOTATION)
            .allowEmptyShould(true)
    ...
    static final ArchRule entities_with_audit_fields_should_declare_entity_listeners = classes()
            ...
            .should().beAnnotatedWith(ENTITY_LISTENERS_ANNOTATION)
            .allowEmptyShould(true)
        ...
        return new ArchCondition<>("declare UUID @Id fields with @UUIDv7Id or UUIDv7Generator") {
            ...
            public void check(JavaClass item, ConditionEvents events) {
                ...
                boolean dependsOnUuidV7Generator = item.getDirectDependenciesFromSelf().stream()
                        .anyMatch(dep -> UUIDV7_GENERATOR.equals(dep.getTargetClass().getName()));

                for (JavaField idField : idFields) {
                    if (!idField.getRawType().isEquivalentTo(UUID.class)) {
                        continue; // legacy non-UUID identifiers are handled in module migrations
                    }
