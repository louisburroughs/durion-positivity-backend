...
public class OutboxPublisher {
    ...
    @Value("${pos.vehicle-inventory.outbox.send-timeout-ms:10000}")
    ...
    public OutboxPublisher(
        ...
        this.publishedCounter = registry == null
                ? null
                : Counter.builder("vehicle.outbox.published")
                        .description("Outbox events successfully published to Kafka")
                        .register(registry);
        this.failedCounter = registry == null
                ? null
                : Counter.builder("vehicle.outbox.publish.failures")
                        .description("Outbox publish attempts that failed")
                        .register(registry);
    }

    @Scheduled(fixedDelayString = "${pos.vehicle-inventory.outbox.poll-interval-ms:1000}")
