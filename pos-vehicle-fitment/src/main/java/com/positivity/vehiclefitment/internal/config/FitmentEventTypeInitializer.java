...
/**
 * Initializes all event types for the pos-vehicle-fitment module on application
 * startup.
 * Uses the pos-events upsert endpoint for idempotent registration.
 */
@Component
public class FitmentEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(FitmentEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;

    public FitmentEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-vehicle-fitment");
    }

    @Override
    public void run(ApplicationArguments args) {
        log.info("Registering {} fitment event types", FitmentEventTypes.all().size());

        initializerSupport.registerEventTypes(
                FitmentEventTypes.all(),
                this::registerEventType);

        log.info("Fitment event type registration complete");
    }
