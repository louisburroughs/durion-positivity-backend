...
class VehicleApplicabilityHintAuditTest {
     ...
     * from the SecurityContext — NOT the {@code createdBy} field from the request body.
     *
        * <p>Asserts that {@code createdBy} is sourced from Spring SecurityContext,
        * satisfying ADR-0018 actor resolution.</p>
     *
    ...
    void givenAuthenticatedUser_whenCreateHint_thenCreatedByIsFromSecurityContext() {
        ...
        ArgumentCaptor<VehicleApplicabilityHint> captor =
                ArgumentCaptor.forClass(VehicleApplicabilityHint.class);
        verify(hintRepository).save(captor.capture());

        assertThat(captor.getValue().getCreatedBy())
                .as("ADR-0018: createdBy MUST be sourced from SecurityContext ('%s'), not request body ('%s')",
     ...
     * from the SecurityContext — NOT the {@code updatedBy} field from the request body.
     *
        * <p>Asserts that {@code updatedBy} is sourced from Spring SecurityContext,
        * satisfying ADR-0018 actor resolution.</p>
     *
    ...
    void givenAuthenticatedUser_whenUpdateHint_thenUpdatedByIsFromSecurityContext() {
        ...
        UpdateHintRequest updateRequest = new UpdateHintRequest(
                List.of(new FitmentTagDto(TagType.MODEL, "Corolla")));
        ...
        assertThat(captor.getValue().getUpdatedBy())
                .as("ADR-0018: updatedBy MUST be sourced from SecurityContext ('%s'), not request body ('%s')",
                        securityContextUser, requestBodyUser)
                .isEqualTo(securityContextUser)
                .isNotEqualTo(requestBodyUser);
    }
     ...
     *
        * <p>Asserts that the controller emits the canonical create event ID
        * {@code VEHICLE_HINT_CREATED} required by Story #44 AC1.</p>
     *
    ...
    void createController_hasEmitEventAnnotation_forCreate() throws Exception {
        Method createHint = VehicleApplicabilityHintController.class
                .getMethod("createHint", com.positivity.vehiclefitment.internal.dto.CreateHintRequest.class);
     ...
     *
        * <p>Asserts that the controller emits the canonical update event ID
        * {@code VEHICLE_HINT_UPDATED} required by Story #44 AC2.</p>
     *
    ...
    void updateController_hasEmitEventAnnotation_forUpdate() throws Exception {
        Method updateHint = VehicleApplicabilityHintController.class
                .getMethod("updateHint", UUID.class,
                        com.positivity.vehiclefitment.internal.dto.UpdateHintRequest.class);
     ...
     *
        * <p>Asserts that the controller emits the canonical delete event ID
        * {@code VEHICLE_HINT_DELETED} required by Story #44.</p>
     *
    ...
    void deleteController_hasEmitEventAnnotation_forDelete() throws Exception {
        Method deleteHint = VehicleApplicabilityHintController.class
                .getMethod("deleteHint", UUID.class);
