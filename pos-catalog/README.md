...
`ServiceEntity` is the system of record for estimated service time (ADR-0058/0059). The `service` table carries the operation taxonomy — `operation_code` (Durion-owned identity, unique when present), `operation_category` (`REPAIR`/`DIAGNOSTIC`/`MAINTENANCE`/`TIRE_SERVICE`) and `default_labor_hours` (vehicle-agnostic fallback only) — authored through `/v1/catalog-items/service`. Vehicle-specific book times live in `service_labor_standard` (V18): year/make/model/submodel/engine key with null-as-wildcard, decimal hours in tenths, `time_type` (retail vs OEM-warranty vs manufacturer-install vs Durion standard), overlap/included-operation metadata, and non-negotiable source + revision provenance. Rows are append-and-supersede: corrections replace a row rather than update it, so a quoted number stays explainable. Only `DURION`-source rows are writable through the API; imported rows are corrected by their source's next import.

**Sourcing (Phase 1, #1569):** labor times arrive through the `LaborTimeProviderPort` SPI (`internal/spi`, pos-supplier adapter pattern) from providers configured under `pos.catalog.labor-guide.providers` — in Phase 1 the `pos-reference-mock` vendor, registered twice: `MOCKGUIDE` (STORE mode, chunked-manifest import into `service_labor_standard` with idempotent chunks, supersede-on-change, and an unmapped-operation curation queue backed by `service_operation_xref`) and `MOCKGUIDE_LIVE` (QUERY_ONLY mode: live lookups under a license-term TTL cache, never persisted). `LaborTimeResolutionService` answers quotes: stored rows most-specific-first with `labor_time_source_policy` precedence, then live QUERY_ONLY sources, then the service's `default_labor_hours` (graded `DEFAULT_HOURS`), then a typed miss. pos-workorder reaches it through the granted `ServiceLaborTimeService` (`catalog.service`, ADR-0044 amendment 2026-09-02) and falls back to the `defaultLaborHours` now riding `catalog.service.updated` schema v2. prod/alpha set `pos.catalog.labor-guide.enabled: false` until a licensed source exists (Phase 2).
...
## Multitenancy (ADR-0062, WS3 wave 4)
...
`pos_app` with no bypass), both on Testcontainers Postgres (`./mvnw -pl pos-catalog -am verify`).
...
Uses Flyway with PostgreSQL. Migrations at `src/main/resources/db/migration`: `V1__baseline_catalog.sql` (the
2026-09-09 flattened baseline with the tenancy schema on every scoped table), `V2__event_outbox_tenant_id.sql`
(`tenant_id` as data on the global outbox table, see Multitenancy below) and the repeatable reference seeds,
each of which binds the alpha default tenant for its own transaction.
