...
class LocationPricingContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void lc102_rejectLifecycle() throws Exception {
        ...
        mockMvc.perform(withAuth(post("/v1/products/pricing/location-overrides/{overrideId}/reject", overrideId))
                        ...
                        .content(objectMapper.writeValueAsString(Map.of(
                                ...
                                approver,
                                "rejectionReasonCode",
                                "PRICE_POLICY_EXCEPTION",
                                "rejectionNotes",
                                "Price reduction requires regional review"))))
                .andExpect(status().isOk())
    ...
    private void upsertGuardrailPolicy(
            UUID locationId, double minMarginPercent, double maxDiscountPercent, double autoApprovalThresholdPercent)
            throws Exception {
