...
class ProductSearchContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void testSearch_noMatchingProducts_returnsEmptyDataWithNullNextCursor() throws Exception {
        ...
        mockMvc.perform(withAuth(MockMvcRequestBuilders.get(SEARCH_PATH).param("q", uniqueTerm)))
    ...
    void testSearch_cursorUsedForNextPage_returnsNextPageItems() throws Exception {
        ...
        MvcResult firstPage = mockMvc.perform(withAuth(MockMvcRequestBuilders.get(SEARCH_PATH)
                        .param("q", namePrefix)
                        .param("limit", "2")))
                .andExpect(status().isOk())
                .andReturn();
        ...
        assertThat(cursor)
                .as("cursor must be non-null for second page navigation")
                .isNotBlank();

        // Second page
        MvcResult secondPage = mockMvc.perform(withAuth(MockMvcRequestBuilders.get(SEARCH_PATH)
                        .param("q", namePrefix)
                        .param("limit", "2")
                        .param("cursor", cursor)))
                .andExpect(status().isOk())
    ...
    void testSearch_queryMatchesDescription_returnsMatchingProduct() throws Exception {
        ...
        Map<String, Object> payload = Map.of(
                ...
                genericName,
                "description",
                "This product has token " + uniqueDescToken + " in its description",
                "unitOfMeasure",
                "EA",
                "manufacturerId",
                ...
                sku,
                "mpn",
                "MPN-CS009-" + UUID.fromString("00000000-0000-0000-0000-000000000001"),
                "upc",
                "1000000000000",
                "attributes",
                "{}");
        ...
        MvcResult result = mockMvc.perform(
                        withAuth(MockMvcRequestBuilders.get(SEARCH_PATH).param("q", uniqueDescToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").isArray())
                .andReturn();
    ...
    private UUID createProductAndGetId(String name, String sku, String mpn) throws Exception {
        Map<String, Object> payload = Map.of(
                ...
                name,
                "description",
                "Product for catalog search contract test",
                "unitOfMeasure",
                "EA",
                "manufacturerId",
                UUID.fromString("00000000-0000-0000-0000-000000000001").toString(),
