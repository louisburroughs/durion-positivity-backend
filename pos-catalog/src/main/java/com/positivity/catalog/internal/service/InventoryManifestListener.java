...
/**
 * Consumer-side reconciliation for the {@code ext_inventory_availability} and {@code ext_product_lead_time} replicas (ADR-0044 §4, issue #899).
 *
 ...
 *
 * <p>The owner's manifest counts every fact in the window (including storage-location on-hand facts this
...
public class InventoryManifestListener {
    ...
    @Value("${pos.catalog.kafka.inventory-commands-topic:inventory.commands.v1}")
    ...
    @KafkaListener(
            topics = "${pos.catalog.kafka.inventory-manifest-topic:inventory.manifest.v1}",
            groupId = "${pos.catalog.kafka.inventory-manifest-consumer-group:pos-catalog-inventory-manifests}")
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=inventory window=[{}, {}): manifests"
