...
/**
 * Drains {@code event_outbox} to Kafka (ADR-0044 §4, #924).
 *
 * <p>At-least-once: a row is marked published only after the broker acknowledges; a crash between
 * send and mark re-sends on the next poll — consumers dedupe by {@code eventId}. Rows are processed
 * in id order (UUIDv7 = insertion time); the batch stops at the first failure so a struggling broker
 * cannot reorder events, and failed rows retry on every poll with {@code attempts}/{@code last_error}
...
public class OutboxPublisher {
    ...
    @Value("${pos.catalog.outbox.send-timeout-ms:10000}")
    ...
    public OutboxPublisher(
        ...
        this.publishedCounter = registry == null
                ? null
                : Counter.builder("catalog.outbox.published")
                        .description("Outbox events successfully published to Kafka")
                        .register(registry);
        this.failedCounter = registry == null
                ? null
                : Counter.builder("catalog.outbox.publish.failures")
                        .description("Outbox publish attempts that failed")
                        .register(registry);
    }

    @Scheduled(fixedDelayString = "${pos.catalog.outbox.poll-interval-ms:1000}")
