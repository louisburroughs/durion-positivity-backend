...
public class PricingClientImpl implements PricingClient {
        ...
        public PricingClientImpl(
                        ...
                        @Value("${pos.price.base-url:http://pos-price}") String baseUrl) {
        ...
        public Optional<PriceQuoteClientResponse> fetchPrice(UUID productId, UUID locationId, UUID customerTierId) {
                ...
                PriceQuoteServiceResponse response = restClient.post()
                                .uri("/v1/price/quotes")
                                .body(body)
                                .retrieve()
                ...
                if (response.msrp().amount() == null) {
                        log.warn("Price service returned null msrp.amount for productId={}, degrading to unavailable",
                                        productId);
                        return Optional.empty();
                }
        ...
        private record PriceQuoteRequest(
                        UUID productId,
                        int quantity,
                        UUID locationId,
                        UUID customerTierId) {
        ...
        private record PriceQuoteServiceResponse(
                        MoneyValue msrp,
