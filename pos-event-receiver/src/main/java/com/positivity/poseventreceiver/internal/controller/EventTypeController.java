...
public class EventTypeController {
        ...
        @GetMapping
        @Operation(summary = "Get all event types", description = "Retrieve all available event types", tags = {
                        "Event Types" })
        ...
        @GetMapping("/active")
        @Operation(summary = "Get active event types", description = "Retrieve only active event types", tags = {
                        "Event Types" })
        ...
        @GetMapping("/{id}")
        @Operation(summary = "Get event type by ID", description = "Retrieve a specific event type by its unique ID", tags = {
                        "Event Types" })
        ...
        @Operation(summary = "Get event type by code", description = "Retrieve a specific event type by its unique type code", tags = {
                        "Event Types" })
        @ApiResponse(responseCode = "200", description = "Event type found and returned", content = @Content(schema = @Schema(implementation = EventTypeResponse.class)))
        @ApiResponse(responseCode = "404", description = "Event type not found")
        ...
        @Operation(summary = "Create event type", description = "Create a new event type for preregistered events", tags = {
                        "Event Types" })
        ...
        public ResponseEntity<EventTypeResponse> createEventType(
                ...
                try {
                        ...
                        if (created.isEmpty()) {
                                log.warn("Event type with code(mask) already exists: {}",
                                                maskForLog(request.getTypeCode()));
                                return ResponseEntity.badRequest().build();
                        }
        ...
        @Operation(summary = "Upsert event type", description = "Create or update an event type by type code", tags = {
                        "Event Types" })
        ...
        public ResponseEntity<EventTypeResponse> upsertEventType(
                ...
                try {
                        ...
                        log.warn("Invalid upsert event type request for code(mask) {}: {}", maskForLog(typeCode),
                                        e.getMessage());
                        return ResponseEntity.badRequest().build();
                }
        }
        ...
        @Operation(summary = "Update event type", description = "Update an existing event type", tags = {
                        "Event Types" })
        ...
        public ResponseEntity<EventTypeResponse> updateEventType(
                ...
                try {
                        ...
                        log.warn("Invalid update event type request for id(mask) {}: {}", maskForLog(id),
                                        e.getMessage());
                        return ResponseEntity.badRequest().build();
                }
        }
        ...
        @Operation(summary = "Delete event type", description = "Delete an event type by ID", tags = { "Event Types" })
        ...
        public ResponseEntity<Void> deleteEventType(
                ...
                try {
                        ...
                        log.warn("Invalid delete event type request for id(mask) {}: {}", maskForLog(id),
                                        e.getMessage());
                        return ResponseEntity.badRequest().build();
                }
        }
