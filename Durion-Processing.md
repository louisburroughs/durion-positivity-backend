...
## Action Plan
- [ ] Update story text: integrate deletion handling (inactive indefinitely when missing in feed), and disable existing price overrides when location becomes INACTIVE.
- [ ] Adjust acceptance criteria to cover inactive-from-missing-feed and override disabling.
- [ ] Clear Open Questions section by incorporating answers.
- [ ] Update labels on origin story: remove blocked:clarification; set status:needs-review (or ready-for-dev if all resolved).
- [ ] Comment/close clarification #244 referencing updates and outcomes.
