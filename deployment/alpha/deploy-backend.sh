...
#
# Off by default, and a full deploy that does NOT ask for it is the teardown: the marker and
# the anchors are removed from the on-box env file and the override is not applied, so the
# stack comes back on the wall clock and GET /system/time returns 404 again. A config-only
# sync instead INHERITS whatever the box is already running, so a merged compose change
# reaching the box mid-run cannot silently drop a seeding run back to wall time. Asking for
# it there is refused: the anchors are a property of the run, generated once at dispatch.
ACCELERATED="${ACCELERATED:-false}"
case "${ACCELERATED}" in
  true|false) ;;
  *)
    echo "ACCELERATED must be 'true' or 'false' (got '${ACCELERATED}')." >&2
    exit 1
    ;;
esac
if [[ "${ACCELERATED}" == "true" && "${MODE}" != "full" ]]; then
  echo "ACCELERATED=true is only valid for a full deploy (deploy-backend.sh <GITHUB_SHA>), not --config-only." >&2
  echo "A config-only sync keeps whatever clock the box is already running." >&2
  exit 1
fi
...
# Replaces or appends KEY=VALUE in the env file. Values reaching this are already
# constrained to [A-Za-z0-9:+./_-] by their validators, so no quoting is needed and the
# file stays readable, matching BACKEND_TAG / ECR_REGISTRY rather than the quoted secrets.
put_env_entry() {
  local key="$1" value="$2"
  if grep -q "^${key}=" "${ENV_FILE}"; then
...
# ─── Accelerated clock (#2065) ───────────────────────────────────────────────

# The marker plus the five pos.time.accelerated.* anchors docker-compose.accelerated.yml
# interpolates. They live in the env file, not only in this process, because a later
# config-only sync re-runs `docker compose up` with no knowledge of the dispatch that
# started the run — and an unset anchor there would either fail the sync or, worse,
# recreate part of the stack on the wall clock halfway through a seeding run.
ACCELERATED_ENV_KEYS=(
  POS_ACCELERATED
...
# Whether java.time.ZoneId.of() would accept this id. ZoneId takes three shapes, and this
# accepts exactly those: the fixed ids UTC/GMT/UT/Z, a fixed offset (+05:00, -08, +05:30:00),
# and a region id, which is precisely a file in the tz database the JVM reads too.
#
# It is deliberately not a regex over "looks like Region/City": 'Mars/Olympus_Mons' matches
# that and is not a zone. Checking the database is what makes 'not-a-zone' fail here rather
# than at startup, on all 25 services at once.
accelerated_zone_is_valid() {
  local zone="$1"

  case "${zone}" in
    UTC|GMT|UT|Z) return 0 ;;
  esac
  ...
  # A box with no tz database cannot answer the region question. Say so and accept rather
  # than reject a zone that is very likely fine — a false refusal here blocks a deploy for a
  # reason the operator cannot act on, while the startup binding still catches a real typo.
  if [[ ! -d "${ZONEINFO_DIR}" ]]; then
    echo "WARNING: no zone database at ${ZONEINFO_DIR}; cannot confirm '${zone}' is a real zone." >&2
    return 0
  fi

  # The name pattern runs first so a value like '../../etc/passwd' is rejected on its shape
  # rather than probed as a path.
  [[ "${zone}" =~ ^[A-Za-z][A-Za-z0-9_+-]*(/[A-Za-z0-9_+-]+)*$ && -f "${ZONEINFO_DIR}/${zone}" ]]
}

# Validates an ISO-8601 UTC instant and prints its epoch seconds. `date -u -d` accepts a
# great deal this deliberately does not: the anchors are generated by one `date -u
# +%Y-%m-%dT%H:%M:%SZ` at dispatch and compared against each other for the gap below, so
# anything else is a mistake worth naming rather than interpreting.
accelerated_epoch() {
  local key="$1" value="$2" epoch
  if [[ ! "${value}" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$ ]]; then
    echo "ERROR: ${key} must be an ISO-8601 UTC instant like 2026-09-17T12:00:00Z (got '${value}')." >&2
    return 1
  fi
  if ! epoch="$(date -u -d "${value}" +%s 2>/dev/null)"; then
    echo "ERROR: ${key}='${value}' is not a real instant." >&2
    return 1
  fi
  printf '%s' "${epoch}"
}

# Checks the anchors a full accelerated deploy was given, before anything on the host is
# touched. AcceleratedTimeProperties re-checks all of this at startup; the point here is to
# fail in seconds rather than after every image has been pulled and every container
# recreated, with 25 JVMs then crash-looping on a bad configuration binding.
validate_accelerated_anchors() {
  local real_start virtual_start scale zone converge real_epoch virtual_epoch gap_days min_gap

  real_start="$(trim_space "${POS_TIME_ACCELERATED_REAL_START:-}")"
  virtual_start="$(trim_space "${POS_TIME_ACCELERATED_VIRTUAL_START:-}")"
  scale="$(trim_space "${POS_TIME_ACCELERATED_SCALE:-1460}")"
  zone="$(trim_space "${POS_TIME_ACCELERATED_ZONE:-UTC}")"
  converge="$(trim_space "${POS_TIME_ACCELERATED_CONVERGE:-true}")"

  if [[ -z "${real_start}" || -z "${virtual_start}" ]]; then
    echo "ERROR: ACCELERATED=true requires POS_TIME_ACCELERATED_REAL_START and" >&2
    echo "POS_TIME_ACCELERATED_VIRTUAL_START. Generate both once, at dispatch time, and pass" >&2
    echo "the same pair to every service — a service deriving its own anchor is skewed from" >&2
  ...
  real_epoch="$(accelerated_epoch POS_TIME_ACCELERATED_REAL_START "${real_start}")" || exit 1
  virtual_epoch="$(accelerated_epoch POS_TIME_ACCELERATED_VIRTUAL_START "${virtual_start}")" || exit 1
  ...
  if [[ ! "${scale}" =~ ^[0-9]+(\.[0-9]+)?$ ]] || ! awk -v s="${scale}" 'BEGIN { exit !(s > 0) }'; then
    echo "ERROR: POS_TIME_ACCELERATED_SCALE must be a positive number (got '${scale}')." >&2
    exit 1
  fi

  # Convergence is NOT optional on this path, which is why a value of `false` is refused
  # rather than warned about. Virtual time is min(virtualStart + scale * elapsed, now) only
  # while convergence is on; with it off the clock runs past wall time and the run stops
  # back-dating and starts FUTURE-dating — expiries, audit history and financial periods
  # land ahead of real time in a database other people share, and no later deploy undoes a
  # row that is already written. Every claim this deployment makes about not writing
  # future-dated records is a claim about this flag.
  #
  # The compose override still reads the variable, so a local single-machine stack can turn
  # it off deliberately. A shared one cannot.
  if [[ "${converge}" != "true" ]]; then
    echo "ERROR: POS_TIME_ACCELERATED_CONVERGE must be 'true' for an accelerated deploy (got '${converge}')." >&2
    echo "With convergence off, virtual time runs PAST wall time: the stack writes future-dated" >&2
    echo "records into a shared database, which no later deploy can undo." >&2
    exit 1
  fi

  # Wall time advances during catch-up, so a gap only closes when scale exceeds 1. At 1 or
  # below the clock would trail wall time forever and the run would never reach the present.
  if ! awk -v s="${scale}" 'BEGIN { exit !(s > 1) }'; then
    echo "ERROR: POS_TIME_ACCELERATED_SCALE must be greater than 1 (got '${scale}');" >&2
    echo "the ${gap_days}-day gap would never close." >&2
    exit 1
  fi

  if [[ -z "${zone}" ]]; then
    echo "ERROR: POS_TIME_ACCELERATED_ZONE must not be empty." >&2
    exit 1
  fi

  # AcceleratedTimeProperties binds this to a java.time.ZoneId, and an id ZoneId.of() does
  # not know fails the binding at startup — i.e. after all 25 services have been recreated,
  # as 25 crash-looping JVMs. An empty-string check does not catch 'not-a-zone', so check
  # the value is a zone the JVM will actually accept.
  if ! accelerated_zone_is_valid "${zone}"; then
    echo "ERROR: POS_TIME_ACCELERATED_ZONE='${zone}' is not a zone java.time.ZoneId accepts," >&2
    echo "so every service would fail its configuration binding at startup." >&2
    echo "Use an IANA region id from ${ZONEINFO_DIR} (e.g. UTC, America/New_York), or a fixed" >&2
    echo "offset such as +05:00." >&2
    exit 1
  fi

  # The run's identity. The SDK suite's journal refuses to resume a run whose realStart
  # differs from the one it recorded, so these lines are what an operator matches against.
  echo "Accelerated clock: scale ${scale}x, zone ${zone}, converge ${converge}"
  echo "  real-start:    ${real_start}   (wall-clock anchor)"
  echo "  virtual-start: ${virtual_start}   (${gap_days} days earlier)"
  # Convergence is why an accelerated run cannot write future-dated records: virtual time is
  # min(virtualStart + scale * elapsed, now), so it closes the gap and then ticks at 1. It is
  # always on here (refused above otherwise), so this line always applies.
  awk -v g="$((real_epoch - virtual_epoch))" -v s="${scale}" \
    'BEGIN { printf "  converges on wall time after ~%.1f real hours, and never passes it\n", g / (s - 1) / 3600 }'

  ACCELERATED_SCALE="${scale}"
  ACCELERATED_ZONE="${zone}"
  ACCELERATED_CONVERGE="${converge}"
  ACCELERATED_REAL_START="${real_start}"
  ACCELERATED_VIRTUAL_START="${virtual_start}"
}

# Writes the run's anchors to the env file and makes it the single source of truth.
persist_accelerated_env() {
  put_env_entry POS_ACCELERATED true
  put_env_entry POS_TIME_ACCELERATED_SCALE "${ACCELERATED_SCALE}"
  put_env_entry POS_TIME_ACCELERATED_ZONE "${ACCELERATED_ZONE}"
  put_env_entry POS_TIME_ACCELERATED_CONVERGE "${ACCELERATED_CONVERGE}"
  put_env_entry POS_TIME_ACCELERATED_REAL_START "${ACCELERATED_REAL_START}"
  put_env_entry POS_TIME_ACCELERATED_VIRTUAL_START "${ACCELERATED_VIRTUAL_START}"

  # Compose ranks the shell environment ABOVE --env-file, the same trap as #1577's
  # SUPPLIER_AUDIT_ENC_KEY: leaving these exported gives the run two sources of truth that
  # only agree until someone edits the file. Unset them now that the file is authoritative.
  unset POS_TIME_ACCELERATED_SCALE POS_TIME_ACCELERATED_ZONE POS_TIME_ACCELERATED_CONVERGE
  unset POS_TIME_ACCELERATED_REAL_START POS_TIME_ACCELERATED_VIRTUAL_START
}

# The teardown half: an ordinary full deploy removes every trace, so the stack comes back on
# the wall clock and pos-api-gateway's GET /system/time returns 404 again — which is the
# proof non-accelerated integration runs wait for, since their guard aborts on a 200.
clear_accelerated_env() {
  local key had=0
  for key in "${ACCELERATED_ENV_KEYS[@]}"; do
    if grep -q "^${key}=" "${ENV_FILE}"; then
      had=1
  ...
  if [[ "${had}" -eq 1 ]]; then
    echo "Accelerated clock cleared: this deploy restores the wall clock and GET /system/time returns 404."
  fi
}
...
# Whether THIS invocation applies the accelerated override. A full deploy is told by
# ACCELERATED; a config-only sync reads what the last full deploy left in the env file, so it
# neither starts nor ends an accelerated run — it only preserves one in flight.
if [[ "${MODE}" == "full" ]]; then
  USE_ACCELERATED="${ACCELERATED}"
  if [[ "${USE_ACCELERATED}" == "true" ]]; then
    validate_accelerated_anchors
  fi
else
  USE_ACCELERATED=false
  if [[ "$(env_value POS_ACCELERATED)" == "true" ]]; then
    USE_ACCELERATED=true
    echo "Config-only sync on an accelerated stack: keeping the clock anchored at" \
         "$(env_value POS_TIME_ACCELERATED_VIRTUAL_START) (scale $(env_value POS_TIME_ACCELERATED_SCALE)x)."
    for key in POS_TIME_ACCELERATED_REAL_START POS_TIME_ACCELERATED_VIRTUAL_START; do
      if [[ -z "$(env_value "${key}")" ]]; then
        echo "ERROR: ${ENV_FILE} marks this box accelerated but ${key} is missing from it." >&2
        echo "The compose interpolation requires it, so this sync would fail after the pull." >&2
        echo "Re-run the deploy-alpha-accelerated workflow to re-anchor the stack, or run an" >&2
        echo "ordinary build-push-ecr deploy to return it to the wall clock." >&2
        exit 1
      fi
    done
  fi
...
if [[ "${USE_ACCELERATED}" == "true" ]]; then
  if [[ ! -f "${ACCELERATED_OVERRIDE}" ]]; then
    echo "ERROR: accelerated deploy asked for, but ${ACCELERATED_OVERRIDE} is not on this box." >&2
    echo "The deploy workflow uploads it to S3 and pulls it here before running this script;" >&2
    echo "a manual run needs deployment/alpha/docker-compose.accelerated.yml placed there." >&2
    exit 1
  fi
  verify_checksum "accelerated override" "${ACCELERATED_OVERRIDE}" "${ACCELERATED_OVERRIDE_SHA256:-}"
fi
...
# Last of the -f files, so it wins: the accelerated override only adds environment entries,
# and compose merges the mapping rather than replacing it, so every service keeps its
# datasource, Kafka, OTEL and client variables and gains the profile and the five anchors
# (#2065). The anchors themselves are interpolated out of the env file below.
if [[ "${USE_ACCELERATED}" == "true" ]]; then
  COMPOSE_ARGS+=(-f "${ACCELERATED_OVERRIDE}")
fi
