...
validate_accelerated_anchors() {
  ...
  case "${converge}" in
    true|false) ;;
    *)
      echo "ERROR: POS_TIME_ACCELERATED_CONVERGE must be 'true' or 'false' (got '${converge}')." >&2
      exit 1
      ;;
  esac

  # Wall time advances during catch-up, so a gap only closes when scale exceeds 1. With
  # convergence on and a scale of 1 or less the clock would trail wall time forever and the
  # run would never reach the present.
  if [[ "${converge}" == "true" ]] && ! awk -v s="${scale}" 'BEGIN { exit !(s > 1) }'; then
    echo "ERROR: POS_TIME_ACCELERATED_SCALE must be greater than 1 when convergence is enabled" >&2
    echo "(got '${scale}'); the ${gap_days}-day gap would never close." >&2
    exit 1
  fi
  ...
  # min(virtualStart + scale * elapsed, now), so it closes the gap and then ticks at 1.
  if [[ "${converge}" == "true" ]]; then
    ...
    echo "  WARNING: convergence is OFF — virtual time will run past wall time and write" >&2
    echo "  future-dated records into a shared database." >&2
  fi
