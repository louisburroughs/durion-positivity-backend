...
/**
 * Materialised location-scope ancestor sets on the pos-order {@code ext_location} replica
 ...
 * reads it. Mirrors pos-people's and pos-invoice's tests of the same name.
 */
@DataJpaTest(
        properties = {
            "spring.datasource.url=jdbc:h2:mem:pos_order_loc_hier;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE",
...
@DisplayName("pos-order LocationHierarchyService — materialised scope ancestor sets")
class LocationHierarchyServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new LocationHierarchyService(extLocationRepository, extLocationParentReplicaRepository);
    ...
    private void locationFact(UUID locationId, long version, String... edges) {
        ...
        listener.onLocationEvent("""
                {"eventId":"evt-%d","eventType":"%s","aggregateVersion":%d,
                 "payload":{"locationId":"%s","code":"L-%d","name":"Loc %s","active":true,
                            "parents":%s}}
                """.formatted(
    ...
    void parentArrivingAfterChildTriggersRecompute() {
        ...
        // ingestion never fails closed on an unknown parent.
        locationFact(SHOP, 1, edge(FIN_MID, "FINANCIAL"));
        assertThat(persisted(SHOP).financial()).containsExactlyInAnyOrder(SHOP, FIN_MID);
        assertThat(extLocationRepository.findById(FIN_MID)).isEmpty();
    ...
    void factWithoutParentsKeepsEdges() {
        ...
        listener.onLocationEvent("""
                {"eventId":"evt-legacy","eventType":"%s","aggregateVersion":2,
                 "payload":{"locationId":"%s","code":"SHOP-1","name":"Renamed","active":true}}
                """.formatted(LocationUpdatedV1.EVENT_TYPE, SHOP));

        assertThat(persisted(SHOP).other()).containsExactlyInAnyOrder(SHOP, REGION);
        assertThat(extLocationRepository.findById(SHOP).orElseThrow().getName()).isEqualTo("Renamed");
    }

    @Test
    @DisplayName("deleting a location removes its row and its edges; descendants keep the id as an ancestor by edge")
    void deleteRemovesRowAndEdges() {
        locationFact(REGION, 1);
        locationFact(SHOP, 1, edge(REGION, "PHYSICAL"));

        listener.onLocationEvent("""
                {"eventId":"evt-del","eventType":"location.location.deleted","aggregateVersion":2,
                 "payload":{"locationId":"%s"}}
                """.formatted(REGION));
        entityManager.flush();
        entityManager.clear();

        assertThat(extLocationRepository.findById(REGION)).isEmpty();
        assertThat(extLocationParentReplicaRepository.findByChildId(REGION)).isEmpty();
