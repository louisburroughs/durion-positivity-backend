...
/**
 * Transactional-outbox writer for order domain events (ADR-0044 §4, plan story D1). Serializes a
 * full {@link DomainEventEnvelope} into {@code event_outbox} within the caller's transaction, so
 * an event exists if and only if the business state change committed. {@link OutboxPublisher}
 * drains the table to Kafka with at-least-once delivery.
 */
@Slf4j
@Component
