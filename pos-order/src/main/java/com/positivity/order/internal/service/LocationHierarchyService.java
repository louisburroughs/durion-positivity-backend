 ...
 * (ADR-0061 §1–§2, issue #1872), mirroring pos-people's and pos-invoice's services of the same
 * name.
 *
...
public class LocationHierarchyService implements LocationAncestorResolver {
     ...
     * fails closed on an unknown parent; the scope check does.
     *
    ...
    public void recomputeAncestors(@NonNull UUID locationId) {
        ...
        for (UUID target : targets) {
            extLocationRepository.findById(target).ifPresent(this::materialise);
        }
    }

    private void materialise(ExtLocation row) {
        ...
        extLocationRepository.save(row);
    }
