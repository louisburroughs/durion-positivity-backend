...
import com.positivity.workorder.internal.service.InventoryManifestListener;
...
class InventoryManifestListenerTest {
    ...
    private String manifestMessage(long eventCount, String checksum) {
        return """
                {
                  "eventId": "%s",
                  "eventType": "inventory.reconciliation.manifest",
                  "schemaVersion": 1,
                  ...
                  "payload": {
                    ...
                    "eventCount": %d,
                    "eventIdsChecksum": "%s",
                    "eventTypeCounts": {"InventoryAvailabilityUpdated": %d}
                  }
                }
                """.formatted(
    ...
    void matchingManifestIsQuiet() {
        ...
        when(repository.findEventIdsInRange(eq("inventory"), anyString(), anyString()))
                .thenReturn(ids);
    ...
    void missingEventTriggersDriftAndReplay() throws Exception {
        ...
        verify(kafkaTemplate).send(eq("inventory.commands.v1"), anyString(), command.capture());
        JsonNode json = objectMapper.readTree(command.getValue());
        assertThat(json.path("commandType").stringValue()).isEqualTo("inventory.outbox.replay-requested");
    ...
    void checksumMismatchTriggersDrift() {
        ...
        assertThat(driftCount()).isEqualTo(1.0);
        verify(kafkaTemplate).send(eq("inventory.commands.v1"), anyString(), anyString());
    }
    ...
    void queriesRepositoryWithWindowBounds() {
        ...
        verify(repository).findEventIdsInRange(eq("inventory"), lower.capture(), upper.capture());
