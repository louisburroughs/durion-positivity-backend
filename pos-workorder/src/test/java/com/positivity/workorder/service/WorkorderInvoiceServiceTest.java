...
class WorkorderInvoiceServiceTest {
    ...
    void generateInvoice_FromCompletedWorkorder_CreatesInvoiceAndPersistsLink() {
        ...
        when(workorderPartRepository.findByWorkorderIdAndWorkOrderServiceIsNull(workorderId))
                .thenReturn(List.of());
        when(idempotencyService.getExistingInvoiceId(IDEMPOTENCY_OPERATION_WORKORDER_INVOICE_GENERATE, INV_KEY_1))
        ...
        verify(idempotencyService)
                .registerInvoiceKey(IDEMPOTENCY_OPERATION_WORKORDER_INVOICE_GENERATE, INV_KEY_1, invoiceId);
    }

    @Test
    @DisplayName(
            "generateInvoice includes traceability links and backfills response links when invoice service omits them")
    ...
    void generateInvoice_DeduplicatesParts_WhenPartHasBothReferences() {
        ...
        BigDecimal totalAmount =
                lineItems.stream().map(InvoiceLineItem::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    void generateInvoice_RaceCondition_ReturnsExistingInvoice() {
        ...
        when(idempotencyService.getExistingInvoiceId(IDEMPOTENCY_OPERATION_WORKORDER_INVOICE_GENERATE, "inv-key-race"))
                .thenReturn(Optional.empty())
        ...
        doThrow(new org.springframework.dao.DataIntegrityViolationException("Duplicate key"))
                ...
                .registerInvoiceKey(IDEMPOTENCY_OPERATION_WORKORDER_INVOICE_GENERATE, "inv-key-race", newInvoiceId);
    ...
    void generateInvoice_ExcludesCancelledItems() {
        ...
        List<String> descriptions =
                lineItems.stream().map(InvoiceLineItem::getDescription).toList();
    ...
    void generateInvoice_IdempotencyKeyReplay_FetchesExistingInvoiceAndUpdatesWorkorder() {
        ...
        when(workorderRepository.findById(workorderId)).thenReturn(Optional.of(workorder));
        when(idempotencyService.getExistingInvoiceId(IDEMPOTENCY_OPERATION_WORKORDER_INVOICE_GENERATE, "retry-key-123"))
                .thenReturn(Optional.of(existingInvoiceId));
