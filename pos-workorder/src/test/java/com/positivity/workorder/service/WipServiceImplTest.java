...
class WipServiceImplTest {
    ...
    void setUp() {
        ...
        lenient()
                .when(workorderRepository.findByShopIdAndStatusIn(
                        any(UUID.class), anyCollection(), any(Pageable.class)))
        ...
        lenient()
                .when(workorderRepository.findByStatusIn(anyCollection(), any(Pageable.class)))
                .thenAnswer(invocation -> {
        ...
        lenient()
                .when(workorderRepository.findById(any(UUID.class)))
                .thenAnswer(invocation -> Optional.of(
                        buildAwaitingPartsWorkorder(invocation.getArgument(0), UUID.fromString(LOCATION_1))));

        lenient()
                .when(stateTransitionRepository.findByWorkorder_Id(any(UUID.class)))
                .thenAnswer(invocation -> List.of(WorkorderStateTransition.builder()
        ...
        lenient()
                ...
                .thenReturn(List.of());
        lenient()
                .when(technicianAssignmentRepository.findByWorkorder_IdAndCurrentTrue(any(UUID.class)))
                .thenReturn(Optional.empty());

        lenient()
                .when(workorderPartRepository.findByWorkorderId(any(UUID.class)))
                .thenReturn(List.of());
        lenient()
                .when(workorderPartRepository.findByWorkOrderService_WorkOrder_Id(any(UUID.class)))
                .thenReturn(List.of());
        lenient()
                .when(workorderServiceRepository.findByWorkOrder_Id(any(UUID.class)))
                .thenReturn(List.of());
        ...
        lenient()
                .when(customerReferenceService.resolve(any(UUID.class)))
                .thenAnswer(invocation ->
    ...
    void getWipWorkorders_multiLocationFalse_scopesToSingleLocation() {
        ...
        assertThat(result.getContent())
                .isNotEmpty()
                .allMatch(
                        v -> locationId.equals(v.getLocationId()),
    ...
    void getWipDetail_partsBlocking_usesQuantityGapRule() {
        ...
        WorkorderPart partReady = buildPart(
                "Oil Filter", new BigDecimal("1.00"), new BigDecimal("1.00"), BigDecimal.ZERO, BigDecimal.ZERO);
        WorkorderPart partBlocked = buildPart(
                "Brake Pad Kit", new BigDecimal("2.00"), new BigDecimal("1.00"), BigDecimal.ZERO, BigDecimal.ZERO);
        ...
        when(workorderPartRepository.findByWorkorderId(workorderId)).thenReturn(List.of(partReady, partBlocked));
    ...
    void getWipDetail_partsBlocking_fallsBackWhenNoQuantityGap() {
        ...
        WorkorderPart coveredPart = buildPart(
                "Cabin Filter", new BigDecimal("1.00"), new BigDecimal("1.00"), BigDecimal.ZERO, BigDecimal.ZERO);
        when(workorderRepository.findById(workorderId))
                .thenReturn(Optional.of(buildAwaitingPartsWorkorder(workorderId, UUID.fromString(LOCATION_1))));
        when(workorderPartRepository.findByWorkorderId(workorderId)).thenReturn(List.of(coveredPart));
        when(workorderPartRepository.findByWorkOrderService_WorkOrder_Id(workorderId))
                .thenReturn(List.of());
    ...
    void getWipWorkorders_currentStatuses_includesAllActiveStatusVariants() {
        ...
        List<WorkorderStatus> returnedStatuses =
                result.getContent().stream().map(WorkorderStatusView::getStatus).toList();
    ...
    private com.positivity.workorder.internal.entity.WorkorderPart buildPart(
            String description, BigDecimal quantity, BigDecimal issued, BigDecimal consumed, BigDecimal returned) {
        return com.positivity.workorder.internal.entity.WorkorderPart.builder()
                .id(UUID.fromString(String.format("00000000-0000-0000-0000-%012d", nextPartUuidSuffix++)))
