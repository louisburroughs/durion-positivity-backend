...
class WorkorderPickFacadeControllerTest {
    ...
    private static final String CONFIRM_LINE_URL =
    ...
    void getPickList_whenServiceReturnsResponse_returns200() throws Exception {
        ...
        var response = WorkorderPickListResponse.builder()
                ...
                .build();
        when(workorderPickFacadeService.getPickListForWorkorder(WORKORDER_ID)).thenReturn(response);
    ...
    void getPickList_whenWorkorderNotFound_returns404() throws Exception {
        ...
        mockMvc.perform(get(PICK_LIST_URL, WORKORDER_ID)).andExpect(status().isNotFound());
    }
    ...
    void getPickList_whenMissingPickListViewAuthority_returns403() throws Exception {
        ...
        // deny
        mockMvc.perform(get(PICK_LIST_URL, WORKORDER_ID).with(req -> {
    ...
    void getPickTasks_whenServiceReturnsTasks_returns200() throws Exception {
        ...
        var task = WorkorderPickTaskResponse.builder()
                ...
                .remainingQty(3)
                .build();
        when(workorderPickFacadeService.getPickTasksForWorkorder(WORKORDER_ID)).thenReturn(List.of(task));
    ...
    void confirmPickLine_whenServiceReturnsResponse_returns202() throws Exception {
        ...
        var response = WorkorderPickTaskResponse.builder()
                ...
                .version(0L)
                .build();
        when(workorderPickFacadeService.confirmPickLine(eq(WORKORDER_ID), eq(PICK_TASK_ID), eq(PICK_LINE_ID), any()))
                .thenReturn(response);
    ...
    void completePickTask_whenServiceReturnsResponse_returns200() throws Exception {
        ...
        var request =
                CompletePickTaskRequest.builder().reason("All items picked").build();
        var response = WorkorderPickTaskResponse.builder()
                .pickTaskId(PICK_TASK_ID)
                .status("COMPLETED")
                .requiredQty(3)
    ...
    void resolveScan_whenMissingExecuteAuthority_returns403() throws Exception {
        // Issue CAP-218: @PreAuthorize("hasAuthority('inventory:pick_list:execute')")
        // must deny
        var request = ResolveScanRequest.builder()
    ...
    void confirmPickLine_whenMissingExecuteAuthority_returns403() throws Exception {
        // Issue CAP-218: @PreAuthorize("hasAuthority('inventory:pick_list:execute')")
        // must deny
        var request = ConfirmPickLineRequest.builder().quantityPicked(1).build();
    ...
    void completePickTask_whenMissingExecuteAuthority_returns403() throws Exception {
        // Issue CAP-218: @PreAuthorize("hasAuthority('inventory:pick_list:execute')")
        // must deny
        var request = CompletePickTaskRequest.builder().reason("test").build();
