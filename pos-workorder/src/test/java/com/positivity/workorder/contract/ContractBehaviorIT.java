...
class ContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void testCreateEstimate_HappyPath() throws Exception {
        ...
        mockMvc.perform(withAuthMvc(post("/v1/workorders/estimates")
                ...
                .andExpect(
                        jsonPath("$.estimateNumber").value(org.hamcrest.Matchers.matchesPattern("EST-\\d{4}-\\d{4}")))
    ...
    void testGetEstimate_HappyPath() throws Exception {
        ...
        String estimateId = objectMapper
                .readTree(createResult.getResponse().getContentAsString())
                .get("id")
                .asString();
        mockMvc.perform(withAuthMvc(
                        get("/v1/workorders/estimates/{id}", estimateId).header("X-Correlation-Id", "test-002")))
    ...
    void testCreateEstimate_TotalsMismatch() throws Exception {
        String invalidPayload = createEstimatePayload("CUST-003", "VEH-003", "LOC-003", "100.00", "10.00", "99.99");
        mockMvc.perform(withAuthMvc(post("/v1/workorders/estimates")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void testCreateEstimate_NegativeSubtotal() throws Exception {
        String invalidPayload = createEstimatePayload("CUST-004", "VEH-004", "LOC-004", "-100.00", "0.00", "-100.00");
        mockMvc.perform(withAuthMvc(post("/v1/workorders/estimates")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(invalidPayload)
    ...
    void testEstimate_ValidEnumValuesAndFormat() throws Exception {
        ...
        String estimateNumber = objectMapper
                .readTree(result.getResponse().getContentAsString())
                .get("estimateNumber")
                .asString();
        String status = objectMapper
                .readTree(result.getResponse().getContentAsString())
                .get("status")
                .asString();

        assert estimateNumber.matches("EST-\\d{4}-\\d{4}") : "estimateNumber must follow EST-YYYY-#### format";
        assert status.matches("DRAFT|APPROVED|DECLINED|EXPIRED|PENDING_APPROVAL") : "Status must be valid enum value";
    }
    ...
    private String createEstimatePayload(
            String customerId, String vehicleId, String locationId, String subtotal, String taxAmount, String total) {
        ...
        return String.format(
                "{\"customerId\":\"%s\",\"vehicleId\":\"%s\",\"locationId\":\"%s\","
                        ...
                        + "\"crmContactIds\":[]}",
                toUuidString(customerId),
                toUuidString(vehicleId),
                toUuidString(locationId),
                subtotal,
                taxAmount,
                total);
    }
