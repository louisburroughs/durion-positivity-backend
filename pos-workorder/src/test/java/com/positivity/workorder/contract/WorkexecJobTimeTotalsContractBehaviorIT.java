...
class WorkexecJobTimeTotalsContractBehaviorIT extends AbstractWorkexecContractBehaviorIT {
    ...
    @Test
    @DisplayName(
            "AC1: GET job-time-totals with valid params returns grouped totals by technicianId, locationId, localDate")
    void getJobTimeTotalsWithValidParamsReturnsGroupedTotals() throws Exception {
        ...
        assertThat(json.findValuesAsText("localDate")).containsOnly("2026-02-14");
        assertThat(json.findValues("totalJobMinutes").stream()
                        .map(JsonNode::asInt)
                        .toList())
