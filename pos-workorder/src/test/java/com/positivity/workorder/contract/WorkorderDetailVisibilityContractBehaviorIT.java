...
class WorkorderDetailVisibilityContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void testGetDetailWithoutFinancialAuthority() {
        ...
        givenWithAuthorities("workorder:workorder:view", "workorder:labor:add", "workorder:parts:add")
                .when()
                .get("/v1/workorders/{workorderId}/detail", workorderId)
                .then()
                .log()
    ...
    void testCapabilityFlagsAccuracy() {
        ...
        givenWithAuthorities("workorder:workorder:view", "workorder:workorder:start", "workorder:labor:add")
                .when()
                .get("/v1/workorders/{workorderId}/detail", workorderId)
                .then()
                .log()
        ...
        List<com.positivity.workorder.internal.entity.WorkorderServiceLine> services =
                workorderServiceRepository.findAll().stream()
                        .filter(s -> s.getWorkOrder() != null
                                && s.getWorkOrder().getId().equals(workorderId))
                        .toList();
