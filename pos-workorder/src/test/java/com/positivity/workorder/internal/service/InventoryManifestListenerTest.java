...
class InventoryManifestListenerTest {
    ...
    void matchingManifestIsQuiet() {
        ...
        when(repository.findEventIdsInRange(eq("inventory"), eq(TENANT_A), anyString(), anyString()))
                .thenReturn(ids);
    ...
    void missingEventTriggersDriftAndReplay() throws Exception {
        ...
        assertThat(replay.topic()).isEqualTo("inventory.commands.v1");
    ...
    void checksumMismatchTriggersDrift() {
        ...
        assertThat(driftCount()).isEqualTo(1.0);
        verify(kafkaTemplate).send(replayOn("inventory.commands.v1"));
    }
    ...
    void queriesRepositoryWithWindowBounds() {
        ...
        verify(repository).findEventIdsInRange(eq("inventory"), eq(TENANT_A), lower.capture(), upper.capture());
