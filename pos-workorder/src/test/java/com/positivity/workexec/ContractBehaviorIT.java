package com.positivity.workexec;
...
class ContractBehaviorIT {
    ...
    void testCreateEstimate_HappyPath() throws Exception {
        ...
        mockMvc.perform(post("/v1/workorders/estimates")
                ...
                .andExpect(jsonPath("$.estimateNumber").value(org.hamcrest.Matchers.matchesPattern("EST-\\d{4}-\\d{4}")))
    ...
    void testGetEstimate_HappyPath() throws Exception {
        ...
        String estimateId = objectMapper.readTree(createResult.getResponse().getContentAsString()).get("id").asText();
    ...
    void testEstimate_ValidEnumValuesAndFormat() throws Exception {
        ...
        String estimateNumber = objectMapper.readTree(result.getResponse().getContentAsString()).get("estimateNumber").asText();
        String status = objectMapper.readTree(result.getResponse().getContentAsString()).get("status").asText();

        assert estimateNumber.matches("EST-\\d{4}-\\d{4}") :
            "estimateNumber must follow EST-YYYY-#### format";
