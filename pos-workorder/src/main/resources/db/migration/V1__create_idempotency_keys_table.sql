...
COMMENT ON TABLE idempotency_keys IS 'Tracks idempotency keys for workorder, change request, and labor entry operations to prevent duplicate processing';
COMMENT ON COLUMN idempotency_keys.key_value IS 'Client-provided idempotency key (UUID or other unique string)';
COMMENT ON COLUMN idempotency_keys.workorder_id IS 'ID of the workorder created with this key (nullable)';
COMMENT ON COLUMN idempotency_keys.change_request_id IS 'ID of the change request created with this key (nullable)';
