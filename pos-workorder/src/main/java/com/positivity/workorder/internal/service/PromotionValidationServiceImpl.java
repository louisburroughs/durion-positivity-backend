    ...
    private void validatePromotionPreconditionsInternal(@NonNull UUID estimateId) {
        ...
        Estimate estimate = estimateRepository
                ...
                .orElseThrow(() -> new PromotionValidationException(
                        PromotionErrorCode.ESTIMATE_NOT_FOUND, "Estimate not found: " + estimateId));
        ...
        if (existingWorkorder.isPresent()) {
            ...
            throw new PromotionValidationException(
                    ...
                    String.format("Estimate %s has already been promoted to workorder %s", estimateId, workorderId),
                    workorderId);
        }
        ...
        if (estimate.getStatus() != EstimateStatus.APPROVED) {
            ...
            throw new PromotionValidationException(
                    ...
                    String.format("Estimate status is %s, must be APPROVED for promotion", estimate.getStatus()));
        }
        ...
        List<EstimateItem> approvedItems = estimateItemRepository.findByEstimate_IdAndApprovalStatusAndDeletedFalse(
        ...
        log.info(
                "Promotion preconditions validated successfully for estimate {} with {} approved items",
                estimateId,
                approvedItems.size());
    }
    ...
    public PromotionValidationResult validatePromotionPreconditionsNonThrowing(@NonNull UUID estimateId) {
        try {
            ...
            if (e.getExistingWorkorderId() != null) {
                return PromotionValidationResult.duplicate(e.getExistingWorkorderId(), e.getMessage());
            }
            return PromotionValidationResult.failure(e.getErrorCode().name(), e.getMessage());
        }
    }
}
