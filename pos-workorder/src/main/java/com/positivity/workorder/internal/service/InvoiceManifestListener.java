...
/**
 * Consumer-side reconciliation for the ext_invoice replica (ADR-0044 §4, #900).
 *
 * <p>For every {@link ReconciliationManifestV1} on {@code invoice.manifest.v1}, recomputes the
 * same count + checksum from {@code processed_events} (owner {@code invoice}) — window
 ...
 *
 * <p>Note: this module intentionally processes only the invoice facts on {@code invoice.events.v1}
...
public class InvoiceManifestListener {
    ...
    @KafkaListener(
            topics = "${workorder.kafka.invoice-manifest-topic:invoice.manifest.v1}",
            groupId = "${workorder.kafka.invoice-manifest-consumer-group:pos-workorder-invoice-manifests}")
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=invoice window=[{}, {}): manifests"
    ...
    private void requestReplay(@NonNull ReconciliationManifestV1 manifest, @NonNull UUID tenantId) {
        try {
            ...
            kafkaTemplate.send(TenantKafkaHeaders.record(
                    invoiceCommandsTopic, manifest.windowStartUtc().toString(), command, tenantId));
        } catch (Exception e) {
    ...
    /** Command envelope for the owner's {@code invoice.commands.v1} listener. */
