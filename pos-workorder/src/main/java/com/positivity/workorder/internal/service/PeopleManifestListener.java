 ...
 * This closes the loop for the staffing-assignment half of {@link PeopleReplicaEventsListener}
 * the same way the sibling {@code customer}/{@code location} manifest listeners already do for
 * their owners.
 */
@Slf4j
@Component
...
public class PeopleManifestListener {
    ...
    @Value("${workorder.kafka.people-commands-topic:people.commands.v1}")
    ...
    @KafkaListener(
            topics = "${workorder.kafka.people-manifest-topic:people.manifest.v1}",
            groupId = "${workorder.kafka.people-manifest-consumer-group:pos-workorder-people-manifests}")
