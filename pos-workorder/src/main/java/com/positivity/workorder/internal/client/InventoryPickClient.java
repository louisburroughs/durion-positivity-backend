...
public class InventoryPickClient {
        ...
        public InventoryPickListDto getPickList(@NonNull UUID pickListId) {
                ...
                if (response == null) {
                        throw new IllegalStateException(
                                        "Inventory service returned an empty response for pick list " + pickListId);
                }
        ...
        public List<InventoryPickListDto> getPickListsForWorkorder(@NonNull UUID workorderId) {
                ...
                if (response == null) {
                        throw new IllegalStateException(
                                        "Inventory service returned an empty pick list collection for workorder "
                                                        + workorderId);
                }
        ...
        public InventoryPickListDto releasePickList(@NonNull UUID pickListId) {
                ...
                if (response == null) {
                        throw new IllegalStateException(
                                        "Inventory service returned an empty response when releasing pick list "
                                                        + pickListId);
                }
        ...
        public List<InventoryPickTaskDto> getPickTasksForPickList(@NonNull UUID pickListId) {
                ...
                if (response == null) {
                        throw new IllegalStateException(
                                        "Inventory service returned an empty pick task collection for pick list "
                                                        + pickListId);
                }
        ...
        public InventoryPickTaskDto confirmPickTask(
                        @NonNull UUID pickListId, @NonNull UUID taskId,
                ...
                InventoryPickTaskDto response = inventoryServiceRestClient
                                .post()
                                .uri("/inventory/v1/inventory/pick-lists/{pickListId}/tasks/{taskId}/confirm",
                                                pickListId, taskId)
                                .body(request)
                                .retrieve()
                ...
                if (response == null) {
                        throw new IllegalStateException(
                                        "Inventory service returned an empty response when confirming pick task "
