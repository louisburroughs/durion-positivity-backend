    ...
    private void copyDeclinedEstimateServicesToWorkorder(Workorder workorder) {
        ...
        List<com.positivity.workorder.internal.entity.WorkorderServiceLine> declinedLines = declinedItems.stream()
                ...
                .map(item -> com.positivity.workorder.internal.entity.WorkorderServiceLine.builder()
                        ...
                        .guideHours(item.getGuideHours())
                        .guideSourceCode(item.getGuideSourceCode())
                        .guideSourceRevision(item.getGuideSourceRevision())
                        .guideMatchGrade(item.getGuideMatchGrade())
                        .guideOverlapGroup(item.getGuideOverlapGroup())
                        .guideIncludedOpCodes(item.getGuideIncludedOpCodes())
                        .rateHourly(item.getRateHourly())
                        .rateBaseHourly(item.getRateBaseHourly())
                        .rateCurrency(item.getRateCurrency())
                        .rateScope(item.getRateScope())
                        .rateId(item.getRateId())
                        .rateAdjustmentCodes(item.getRateAdjustmentCodes())
            ...
            if (estimateItem.getItemType() == EstimateItemType.LABOR) {
                ...
                com.positivity.workorder.internal.entity.WorkorderServiceLine workorderService =
                        com.positivity.workorder.internal.entity.WorkorderServiceLine.builder()
                                ...
                                .guideHours(estimateItem.getGuideHours())
                                .guideSourceCode(estimateItem.getGuideSourceCode())
                                .guideSourceRevision(estimateItem.getGuideSourceRevision())
                                .guideMatchGrade(estimateItem.getGuideMatchGrade())
                                .guideOverlapGroup(estimateItem.getGuideOverlapGroup())
                                .guideIncludedOpCodes(estimateItem.getGuideIncludedOpCodes())
                                .rateHourly(estimateItem.getRateHourly())
                                .rateBaseHourly(estimateItem.getRateBaseHourly())
                                .rateCurrency(estimateItem.getRateCurrency())
                                .rateScope(estimateItem.getRateScope())
                                .rateId(estimateItem.getRateId())
                                .rateAdjustmentCodes(estimateItem.getRateAdjustmentCodes())
