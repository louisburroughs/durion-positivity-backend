 ...
 * facts (ADR-0044 §6), mirroring pos-inventory's and pos-people's replicas of the same name. The
 * owner allows exactly one parent per (child, parentType), so that pair is the natural key; every
 * location fact replaces the child's full edge set.
 *
 * <p>Feeds the materialised location-scope ancestor sets on {@link ExtLocationReplica}
 * (ADR-0061 §2, #1878): the upward walk follows these edges by type, and the downward walk finds
