...
- **Multitenancy (ADR-0062, accepted 2026-09-09; schema in place, runtime landing module by module).** Every persisting
  ...
  `pos-security-service`, `pos-inventory`, `pos-accounting`, `pos-workorder` so far; WS3 waves continue largest first) and enforced by `pos-archunit`'s `TenancyArchitectureTest` for the modules in
