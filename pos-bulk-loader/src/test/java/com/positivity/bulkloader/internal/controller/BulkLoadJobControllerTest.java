...
class BulkLoadJobControllerTest {
    ...
    void retryJob_whenNotFailed_returns409() throws Exception {
        ...
        mockMvc.perform(post("/v1/bulk-jobs/{jobId}/retry", JOB_ID)).andExpect(status().isConflict());
    }

    @Test
    void retryJob_withoutExecuteAuthority_returns403() throws Exception {
        mockMvc.perform(post("/v1/bulk-jobs/{jobId}/retry", JOB_ID).header("X-Authorities", "bulkImport:status:read"))
                .andExpect(status().isForbidden());
    }
    ...
    void retryJob_whenNotFound_returns404() throws Exception {
        ...
        mockMvc.perform(post("/v1/bulk-jobs/{jobId}/retry", JOB_ID)).andExpect(status().isNotFound());
    }
