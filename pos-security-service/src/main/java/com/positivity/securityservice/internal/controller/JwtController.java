...
public class JwtController {
        ...
        public ResponseEntity<TokenResponse> issueInternalToken(@Valid @RequestBody InternalTokenRequest request) {
                ...
                var user = userService
                                ...
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "User not found for subject: " + request.subject()));
                if (user.getId() == null) {
                        throw new IllegalStateException(
                                        "User exists but id is missing for subject: " + request.subject());
                }
         ...
         * @throws IllegalArgumentException if username is blank or roles are empty
         */
        @Operation(summary = "Issue privileged JWT token pair (access + refresh)", description = "Privileged internal endpoint that issues both access token (1-hour) and refresh token (7-day). "
        ...
        @ApiResponse(responseCode = "400", description = "Invalid request (blank username or empty roles)", content = @Content(schema = @Schema(implementation = ApiError.class)))
        @ApiResponse(responseCode = "403", description = "Forbidden: missing security:token:issue_internal permission", content = @Content(schema = @Schema(implementation = ApiError.class)))
        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(schema = @Schema(implementation = ApiError.class)))
        @io.swagger.v3.oas.annotations.security.SecurityRequirement(name = "bearerAuth", scopes = {
                        "security:token:issue_internal" })
        ...
        public ResponseEntity<TokenPairResponse> generateTokenPair(@Valid @RequestBody TokenPairRequest request) {
                ...
                JwtService.TokenPair tokenPair = jwtService.generateTokenPair(
                                request.subject(), user.getId(), null,
