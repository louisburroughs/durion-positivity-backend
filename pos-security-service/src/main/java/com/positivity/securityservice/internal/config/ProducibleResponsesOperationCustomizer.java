 ...
 * onto <em>every</em> operation from this module's unscoped {@code @ControllerAdvice}
 * ({@link GlobalExceptionHandler}), keeping only the codes an operation can actually produce (issue #1721).
 *
 ...
 * responses regardless of whether that operation's handler can ever throw the corresponding exception. Because
 * {@code GlobalExceptionHandler} is unscoped, its {@code 400} ({@code handleBadRequestExceptions},
 * {@code handleSecurityValidationException}), {@code 401} (credential/token handlers), {@code 403}
 * ({@code handleAuthorizationDeniedException}), {@code 404} (not-found handlers), and {@code 409} (conflict
 * handlers) land on all 71 operations in {@code pos-security-service}, even endpoints that are {@code permitAll()}
 * with no request body (which can never 401/403/404/409) or that take no parameters at all (which can never 400).
 * This misrepresents the actual contract to API consumers and contract tests. See ADR-0017 §1 (Canonical HTTP
 ...
 * {@code 404} is a missing resource, {@code 409} is a target-resource identity/version/lifecycle collision.
 *
 ...
 *   <li>a {@code 2xx} code, or {@code "default"} — springdoc's success responses are never touched;
 *   <li>{@code "400"}, when the operation has at least one parameter or a request body — binding/validation
 *       failures ({@code GlobalExceptionHandler#handleBadRequestExceptions}) are only possible then;
 ...
 *       security can then deny an authenticated caller
 *       ({@code GlobalExceptionHandler#handleAuthorizationDeniedException}). {@code isAuthenticated()} alone
 *       cannot produce {@code 403} (the filter chain answers {@code 401} first), and neither can
 *       {@code permitAll()}.
 * </ul>
 *
 ...
 * the class level; if neither is present the operation is treated as authenticated-only, matching
 * {@code SecurityConfig}'s {@code anyRequest().authenticated()} default.
 *
