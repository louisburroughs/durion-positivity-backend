...
/**
 * Consumer-side reconciliation for the staffing-assignment read model (ADR-0044 §4, #1867).
 *
 * <p>For every {@link ReconciliationManifestV1} on {@code people.manifest.v1}, recomputes the same
 * count + checksum from {@code processed_events} (owner {@code people}) — window membership is
 * the UUIDv7 timestamp embedded in each recorded eventId, exactly the definition the owner used.
 * On mismatch it increments {@code replica.drift}
 * (Prometheus: {@code replica_drift_total{owner="people"}}) and publishes a
 * {@code people.outbox.replay-requested} command on {@code people.commands.v1}; the replayed
 * events are deduplicated by the {@code processed_events} primary key, so repair is idempotent.
 * pos-people's {@code ManifestPublisher} and {@code PeopleCommandListener} already provide the
 * producer half.
 *
 * <p>The comparison is stateless: reprocessing a manifest yields the same verdict, and a
 * duplicate replay request only causes an idempotent re-delivery.
 *
...
public class PeopleManifestListener {
    ...
    @Value("${pos.security-service.kafka.people-commands-topic:people.commands.v1}")
    ...
    @KafkaListener(
            topics = "${pos.security-service.kafka.people-manifest-topic:people.manifest.v1}",
            groupId = "${pos.security-service.kafka.people-manifest-consumer-group:pos-security-people-manifests}")
    ...
    private void requestReplay(@NonNull ReconciliationManifestV1 manifest) {
        try {
            ...
            kafkaTemplate.send(TenantKafkaHeaders.record(
                    peopleCommandsTopic, manifest.windowStartUtc().toString(), command, manifest.tenantId()));
        } catch (Exception e) {
