...
/**
 * Consumer-side reconciliation for the customer person-identity replica (ADR-0044 §4, #891).
 *
...
public class CustomerManifestListener {
    ...
    @Value("${pos.security-service.kafka.customer-commands-topic:customer.commands.v1}")
    ...
    @KafkaListener(
            topics = "${pos.security-service.kafka.customer-manifest-topic:customer.manifest.v1}",
            groupId = "${pos.security-service.kafka.customer-manifest-consumer-group:pos-security-customer-manifests}")
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=customer window=[{}, {}): manifests"
