...
public record CustomerPersonSearchResponse(
        ...
        @Schema(
                description = "True when the person is an individual customer",
                example = "true",
                requiredMode = Schema.RequiredMode.REQUIRED)
        boolean individualCustomer,

        @Schema(
                description = "True when the person is a commercial contact",
                example = "false",
                requiredMode = Schema.RequiredMode.REQUIRED)
        boolean commercialContact,

        @Schema(
                description = "Number of commercial accounts the person is linked to",
                example = "0",
                requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    public record ContactPointDto(
            @Schema(
                    description = "Contact point identifier",
            ...
            @Schema(
                    description = "True when this is the primary contact point of its type",
                    example = "true",
                    requiredMode = Schema.RequiredMode.REQUIRED)
            boolean isPrimary) {}
}
