...
public class OutboxPublisher {
    ...
    @Value("${pos.security-service.outbox.send-timeout-ms:10000}")
    ...
    public OutboxPublisher(
        ...
        this.publishedCounter = registry == null
                ? null
                : Counter.builder("security.outbox.published")
                        .description("Outbox events successfully published to Kafka")
                        .register(registry);
        this.failedCounter = registry == null
                ? null
                : Counter.builder("security.outbox.publish.failures")
                        .description("Outbox publish attempts that failed")
                        .register(registry);
    }

    @Scheduled(fixedDelayString = "${pos.security-service.outbox.poll-interval-ms:1000}")
    @PlatformScoped(
            reason =
                    "drains event_outbox, a global table; each row carries its own tenant, stamped on the record header")
