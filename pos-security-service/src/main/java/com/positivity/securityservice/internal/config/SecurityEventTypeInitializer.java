...
/**
 * Initializes all event types for the pos-security-service module on
 * application startup.
 * Uses the pos-events upsert endpoint for idempotent registration.
 */
@Component
public class SecurityEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(SecurityEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;
    private final String apiSecret;

    public SecurityEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-security-service");
        this.apiSecret = apiSecret;
    }

    @Override
    public void run(ApplicationArguments args) {
        log.info("Registering {} security event types", SecurityEventTypes.all().size());

        initializerSupport.registerEventTypes(
                SecurityEventTypes.all(),
                this::registerEventType);

        log.info("Security event type registration complete");
    }
