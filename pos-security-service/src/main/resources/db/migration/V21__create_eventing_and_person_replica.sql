-- ADR-0044 §4 / amended ADR-0043 (#876): eventing infrastructure for the users.person_id
-- projection and local person resolution.

-- Transactional outbox: people-contact.commands.v1 person/link commands.
...
-- Idempotency log for the people-contact.events.v1 consumer.
...
-- Read-only person identity replica for local self-registration person resolution
-- (weighted matching against names/contacts; pos-people-contact owns the facts).
...
CREATE INDEX idx_sec_ext_person_primary_email ON ext_people_contact_person (primary_email);
CREATE INDEX idx_sec_ext_person_last_name ON ext_people_contact_person (last_name);
