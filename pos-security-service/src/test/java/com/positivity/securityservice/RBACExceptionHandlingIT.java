...
class RBACExceptionHandlingIT extends BaseIntegrationTest {
    ...
    void testGetRoleByName_NotFound() throws Exception {
        String nonExistentRoleName = "NON_EXISTENT_ROLE_" + UUID.fromString("00000000-0000-0000-0000-000000000001");

        MvcResult result = mockMvc.perform(withAuth(get("/v1/roles/by-name/" + nonExistentRoleName))
        ...
        ApiError errorResponse = objectMapper.readValue(result.getResponse().getContentAsString(), ApiError.class);
    ...
    void testGetUserRoleAssignments_UserNotFound() throws Exception {
        ...
        MvcResult result = mockMvc.perform(withAuth(get("/v1/roles/assignments/user/" + nonExistentUserId))
                        .header("X-Correlation-Id", TEST_CORRELATION_ID)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void testGetUserPermissions_UserNotFound() throws Exception {
        ...
        MvcResult result = mockMvc.perform(withAuth(get("/v1/roles/permissions/user/" + nonExistentUserId))
                        .header("X-Correlation-Id", TEST_CORRELATION_ID)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void testCheckUserPermission_UserNotFound() throws Exception {
        ...
        MvcResult result = mockMvc.perform(withAuth(get("/v1/roles/check-permission")
                        .header("X-Correlation-Id", TEST_CORRELATION_ID)
