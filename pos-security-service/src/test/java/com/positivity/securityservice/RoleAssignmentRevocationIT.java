...
class RoleAssignmentRevocationIT extends BaseIntegrationTest {
    ...
    void testImmediateRevocation() throws Exception {
        ...
        RoleAssignment assignment = createAssignment(LocalDateTime.now(TEST_CLOCK), null, ScopeType.GLOBAL);

        Instant beforeRevocation = Instant.now();

        // When: Revoke immediately (no endDate parameter = defaults to today)
        mockMvc.perform(withAuth(delete("/v1/roles/assignments/" + assignment.getId()))
        ...
        RoleAssignment revokedAssignment =
                roleAssignmentRepository.findById(assignment.getId()).orElseThrow();
    ...
    void testBackdatedRevocation() throws Exception {
        ...
        RoleAssignment assignment =
                createAssignment(LocalDateTime.now(TEST_CLOCK).minusDays(30), null, ScopeType.GLOBAL);
    ...
    void testUpdatingRevocation() throws Exception {
        ...
        RoleAssignment firstRevocation =
                roleAssignmentRepository.findById(assignment.getId()).orElseThrow();
        ...
        RoleAssignment secondRevocation =
                roleAssignmentRepository.findById(assignment.getId()).orElseThrow();
    ...
    void testRevokeNonExistentAssignment() throws Exception {
        ...
        mockMvc.perform(withAuth(delete("/v1/roles/assignments/" + nonExistentId))
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    void testRetrieveRevokedAssignments() throws Exception {
        ...
        mockMvc.perform(withAuth(delete("/v1/roles/assignments/" + assignment.getId()))
                        .param(
                                "endDate",
                                LocalDateTime.now(TEST_CLOCK).minusDays(5).toString())
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNoContent());

        // When: Retrieve assignments without history
        MvcResult withoutHistory = mockMvc.perform(withAuth(get("/v1/roles/assignments/user/" + testUser.getId()))
        ...
        MvcResult withHistory = mockMvc.perform(withAuth(get("/v1/roles/assignments/user/" + testUser.getId()))
     ...
     */
    private RoleAssignment createAssignment(LocalDateTime startDate, LocalDateTime endDate, ScopeType scopeType) {
