...
class AuditControllerTest {
    ...
    void searchAuditEvents_noFilters_returnsPage() throws Exception {
        AuditLogEventDto event = AuditLogEventDto.builder()
                ...
                .build();
        when(auditEventService.searchEventsFiltered(any(), any())).thenReturn(new PageImpl<>(List.of(event)));
    ...
    void searchAuditEvents_missingAuthority_returns403() throws Exception {
        mockMvc.perform(get("/v1/audit/events")).andExpect(status().isForbidden());
    }
    ...
    void getAuditExportJob_unknownJob_returns404() throws Exception {
        ...
        mockMvc.perform(get("/v1/audit/exports/{jobId}", UNKNOWN_JOB_ID)).andExpect(status().isNotFound());
    }
