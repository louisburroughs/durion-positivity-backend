...
class AuditTrailQueryServiceTest {
    ...
    void getByOrderId_returnsMappedResponses() {
        when(auditRepository.findByOrderIdOrderByTimestampAsc(testOrderId))
    ...
    void getByInvoiceId_returnsMappedResponses() {
        when(auditRepository.findByInvoiceIdOrderByTimestampAsc(testInvoiceId))
                .thenReturn(List.of(testEntry));
    ...
    void getByTypeAndDateRange_returnsFilteredEntries() {
        ...
        when(auditRepository.findByExceptionTypeAndDateRange(
                eq(ExceptionType.PRICE_OVERRIDE), eq(start), eq(end)))
                .thenReturn(List.of(testEntry));

        List<AuditTrailResponse> results = service.getByTypeAndDateRange(
                ExceptionType.PRICE_OVERRIDE, start, end);
    ...
    void getByDateRange_returnsEntriesInRange() {
        ...
        when(auditRepository.findByDateRange(eq(start), eq(end)))
                .thenReturn(List.of(testEntry));
