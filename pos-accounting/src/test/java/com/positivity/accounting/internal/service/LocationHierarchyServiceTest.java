 ...
 * test.
 */
@DisplayName("pos-accounting LocationHierarchyService — materialised scope ancestor sets")
class LocationHierarchyServiceTest {
    ...
    void setUp() {
        ...
        when(parentRepository.findByParentId(any()))
                .thenAnswer(i -> edges.stream()
                        ...
                        .toList());
        when(locationRepository.findByCode(any()))
                .thenAnswer(i -> locations.values().stream()
                        .filter(row -> i.getArgument(0).equals(row.getCode()))
                        .findFirst());
    ...
    @Test
    @DisplayName(
            "the GL locationId dimension code resolves to the replicated location id, and an unknown code does not")
    void locationCodeResolvesToId() {
        replicate(SHOP, Map.of());
        locations.get(SHOP).setCode("LOC-107");

        assertThat(service.locationIdForCode("LOC-107")).contains(SHOP);
        assertThat(service.locationIdForCode("LOC-999")).isEmpty();
    }
}
