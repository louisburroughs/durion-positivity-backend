...
class PaymentApplicationControllerIntegrationTest extends BaseIntegrationTest {
    ...
    void testVoidPayment_Success() throws Exception {
        ...
        ReceivablePayment payment =
                receivablePaymentRepository.findById(testPaymentId).orElseThrow();
    ...
    void testApplyPayment_SingleInvoice_Success() throws Exception {
        ...
        request.setApplications(List.of(createInvoiceApplication(testInvoice1Id, "500.00")));
    ...
    void testApplyPayment_PartialApplication() throws Exception {
        ...
        PaymentApplicationRequest request = new PaymentApplicationRequest();
        request.setApplicationRequestId(nextUuid().toString());
        request.setApplications(List.of(createInvoiceApplication(testInvoice1Id, "600.00")));
    ...
    void testApplyPayment_Idempotent() throws Exception {
        ...
        String idempotencyKey = nextUuid().toString();
        PaymentApplicationRequest request = new PaymentApplicationRequest();
    ...
    void testApplyPayment_PaymentNotFound() throws Exception {
        // Arrange
        UUID nonExistentPaymentId = nextUuid();
        PaymentApplicationRequest request = new PaymentApplicationRequest();
        request.setApplicationRequestId(nextUuid().toString());
        request.setApplications(List.of(createInvoiceApplication(testInvoice1Id, "500.00")));
    ...
    void testReversePaymentApplication_Success() throws Exception {
        ...
        mockMvc.perform(withAuth(post(
                                API_V1 + "/payment-applications/" + application.getPaymentApplicationId() + "/reverse"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andDo(print())
                .andExpect(status().isNoContent());
    ...
    void testReversePaymentApplication_NotFound() throws Exception {
        // Arrange
        UUID nonExistentApplicationId = nextUuid();
        PaymentApplicationReversalRequest request = new PaymentApplicationReversalRequest();
        request.setReason("Test reason for reversal");

        // Act & Assert
        mockMvc.perform(withAuth(post(API_V1 + "/payment-applications/" + nonExistentApplicationId + "/reverse"))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andDo(print())
                .andExpect(status().isNotFound());
    ...
    void testReversePaymentApplication_ReasonTooShort() throws Exception {
        ...
        applyRequest.setApplicationRequestId(nextUuid().toString());
