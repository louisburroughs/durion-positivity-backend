...
class APPaymentContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void testExecutePayment_AutomaticAllocation_Success() throws Exception {
        ...
        // bill2 partially)
        APPayment savedPayment =
                apPaymentRepository.findByPaymentRef(paymentRef).orElseThrow();
        assertThat(savedPayment.getUnappliedAmount()).isEqualByComparingTo(BigDecimal.ZERO);

        List<APPaymentAllocation> allocations =
                allocationRepository.findByPayment_PaymentIdOrderByAllocationSequenceAsc(savedPayment.getPaymentId());
    ...
    void testExecutePayment_ExplicitAllocations_Success() throws Exception {
        ...
        assertThat(allocationRepository.findByPayment_PaymentIdOrderByAllocationSequenceAsc(
    ...
    void testExecutePayment_Idempotency_Success() throws Exception {
        ...
        String paymentId1 = objectMapper
        ...
        String paymentId2 = objectMapper
    ...
    void testListEligibleBills_Success() throws Exception {
        ...
        MvcResult result = mockMvc.perform(withAuth(get(API_V1_AP_BILLS)).param("vendorId", testVendorId.toString()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray())
                .andExpect(jsonPath("$.content[0].billNumber").value("BILL-001")) // Oldest due first
                .andExpect(jsonPath("$.content[0].openAmount").value(500.00))
                .andExpect(jsonPath("$.content[1].billNumber").value("BILL-002")) // Newer due second
                .andExpect(jsonPath("$.content[1].openAmount").value(300.00))
    ...
    void testListBills_NullDueDate_AppearsLast() throws Exception {
        ...
        mockMvc.perform(withAuth(get(API_V1_AP_BILLS)).param("vendorId", nullDueVendorId.toString()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray())
    ...
    void testListEligibleBills_EmptyList() throws Exception {
        ...
        mockMvc.perform(withAuth(get(API_V1_AP_BILLS)).param("vendorId", nonExistentVendor.toString()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray())
