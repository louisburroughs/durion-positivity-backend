...
public class AccountingEventSubmitRequest {
    ...
    @Schema(description = "Optional event UUID (idempotency key). If omitted, service computes idempotency hash.",
            example = "01936e5c-1234-7a3d-8b6e-123456789012")
    private UUID eventId;

    @NotBlank(message = "eventType is required")
    @Size(max = 100, message = "eventType must not exceed 100 characters")
    @Schema(description = "Event type identifier", example = "INVOICE_RECEIVED",
    ...
    @Schema(description = "Source system name. If omitted, defaults to POS_ACCOUNTING_API.",
            example = "POS_ORDER")
    private String sourceSystem;

    @Schema(description = "Event transaction timestamp. If omitted, defaults to current time.",
