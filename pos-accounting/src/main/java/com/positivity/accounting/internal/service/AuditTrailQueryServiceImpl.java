...
public class AuditTrailQueryServiceImpl implements AuditTrailQueryService {
    ...
    public List<AuditTrailResponse> getByOrderId(UUID orderId) {
        ...
        return auditRepository.findByOrderIdOrderByTimestampAsc(orderId).stream()
    ...
    public List<AuditTrailResponse> getByInvoiceId(UUID invoiceId) {
        ...
        return auditRepository.findByInvoiceIdOrderByTimestampAsc(invoiceId).stream()
                .map(this::mapToResponse)
                .toList();
    }
     ...
     */
    public List<AuditTrailResponse> getByTypeAndDateRange(ExceptionType type, Instant startDate, Instant endDate) {
        log.info("Querying audit entries for type {} between {} and {}", type, startDate, endDate);
        return auditRepository.findByExceptionTypeAndDateRange(type, startDate, endDate).stream()
                .map(this::mapToResponse)
                .toList();
    }
     ...
     */
    public List<AuditTrailResponse> getByActorAndDateRange(String actorId, Instant startDate, Instant endDate) {
        log.info("Querying audit entries for actor {} between {} and {}", actorId, startDate, endDate);
        return auditRepository.findByActorAndDateRange(actorId, startDate, endDate).stream()
                .map(this::mapToResponse)
                .toList();
    }
    ...
    public List<AuditTrailResponse> getByDateRange(Instant startDate, Instant endDate) {
        ...
        return auditRepository.findByDateRange(startDate, endDate).stream()
                .map(this::mapToResponse)
                .toList();
    }
