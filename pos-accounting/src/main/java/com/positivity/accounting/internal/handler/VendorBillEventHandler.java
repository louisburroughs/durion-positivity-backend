...
public class VendorBillEventHandler {
    ...
    public void onGoodsReceived(@NonNull GoodsReceivedEvent event) {
        log.debug("Received GoodsReceivedEvent | eventId={} | vendorId={}", event.getEventId(), event.getVendorId());

        try {
            VendorBillResponse response = vendorBillService.handleGoodsReceivedEvent(event);
            log.info(
                    "Vendor bill created from goods receipt | billId={} | status={}",
                    response.getVendorBillId(),
                    response.getStatus());
        } catch (Exception e) {
            log.error(
                    "Failed to create vendor bill from goods receipt | eventId={} | error={}",
                    event.getEventId(),
                    e.getMessage(),
                    e);
    ...
    public void onVendorInvoiceReceived(@NonNull VendorInvoiceReceivedEvent event) {
        log.debug(
                "Received VendorInvoiceReceivedEvent | eventId={} | vendorId={} | invoiceRef={}",
                event.getEventId(),
                event.getVendorId(),
        ...
        try {
            ...
            log.info(
                    "Three-way match completed | billId={} | status={}",
                    response.getVendorBillId(),
                    response.getStatus());
        } catch (Exception e) {
            log.error(
                    "Failed to process vendor invoice | eventId={} | error={}", event.getEventId(), e.getMessage(), e);
            // In production, would publish failure event for DLQ/retry
            throw e;
        }
    }
