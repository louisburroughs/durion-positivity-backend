...
public class AccountingStatusSyncServiceImpl implements AccountingStatusSyncService {
    ...
    public void processStatusEvent(@NonNull AccountingStatusChangedEvent event) {
        ...
        InvoiceStatusView statusView = statusViewRepository
                .findByInvoiceId(event.getInvoiceId())
        ...
        if (statusView.getAccountingStatusUpdatedAt() != null
            ...
            log.warn(
                    "Out-of-order accounting status event ignored: event timestamp {} is before current updatedAt {} for invoice {}",
                    event.getTimestamp(),
                    statusView.getAccountingStatusUpdatedAt(),
                    event.getInvoiceId());
            return;
        }
        ...
        if (currentStatus != null && isBackwardTransition(currentStatus, event.getNewStatus())) {
            log.warn(
                    "Backward accounting status transition rejected: {} → {}; event {} ignored.",
                    currentStatus,
                    event.getNewStatus(),
                    event.getEventId());
            return;
        ...
        log.info(
                "Processed accounting status event: invoiceId={}, {} → {}, eventId={}",
                event.getInvoiceId(),
                oldStatus,
                event.getNewStatus(),
                event.getEventId());
    ...
    public void recoverProcessStatusEvent(Exception e, @NonNull AccountingStatusChangedEvent event) {
        log.error(
                "Retry exhausted for processStatusEvent: eventId={}, invoiceId={}. Cause: {}",
                event.getEventId(),
                event.getInvoiceId(),
                e.getMessage(),
                e);
    ...
    private @NonNull AccountingStatusView buildAccountingStatusView(@NonNull UUID invoiceId) {
        InvoiceStatusView statusView = statusViewRepository
                ...
                .orElseThrow(
                        () -> new jakarta.persistence.EntityNotFoundException(INVOICE_STATUS_NOT_FOUND + invoiceId));
    ...
    private static boolean isBackwardTransition(AccountingStatus current, AccountingStatus proposed) {
        ...
        Set<AccountingStatus> earlyStates = EnumSet.of(AccountingStatus.PENDING_POSTING, AccountingStatus.ON_HOLD);
