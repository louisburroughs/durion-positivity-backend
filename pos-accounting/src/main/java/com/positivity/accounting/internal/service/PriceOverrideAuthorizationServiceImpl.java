...
public class PriceOverrideAuthorizationServiceImpl implements PriceOverrideAuthorizationService {
    ...
    private static final List<String> FORBIDDEN_CATEGORIES =
            Arrays.asList("BELOW_COST", "STACKING_VIOLATION", "REGULATED_FEE", "WARRANTY_OVERRIDE");
     ...
     */
    @Override
    public AuthorizationResult validate(
            String role, BigDecimal originalPrice, BigDecimal adjustedPrice, String categoryCode) {
        log.info("Validating price override for role {} from {} to {}", role, originalPrice, adjustedPrice);
        ...
        Optional<OverridePolicyThreshold> policyOpt = policyRepository.findActiveByRoleAtTime(role, Instant.now(clock));
        ...
        BigDecimal discountPercent =
                discountAmount.divide(originalPrice, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100));

        // Check absolute amount threshold
        if (policy.getMaxAbsoluteAmount() != null && discountAmount.compareTo(policy.getMaxAbsoluteAmount()) > 0) {
            log.warn(
                    "Price override rejected - exceeds max absolute amount: {} > {}",
                    discountAmount,
                    policy.getMaxAbsoluteAmount());
            return AuthorizationResult.builder()
                    .result(PolicyValidationResult.REJECTED_THRESHOLD_EXCEEDED)
        ...
        if (policy.getMaxPercentOff() != null && discountPercent.compareTo(policy.getMaxPercentOff()) > 0) {
            log.warn(
                    "Price override rejected - exceeds max percentage: {}% > {}%",
        ...
        log.info("Price override approved for role {} under policy version {}", role, policy.getVersion());
        return AuthorizationResult.builder()
