...
public class VendorBillController {
        ...
        @Operation(summary = "Create vendor bill from goods received event", description = "Creates a vendor bill from an inbound goods-received event payload", tags = {
                        "Vendor Bill API" })
        ...
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:ap:pay" })
        @PreAuthorize("hasAuthority('accounting:ap:pay')")
        @Operation(summary = "Match vendor invoice", description = "Performs matching for a received vendor invoice and creates/updates bill state", tags = {
                        "Vendor Bill API" })
        ...
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:ap:pay" })
        @PreAuthorize("hasAuthority('accounting:ap:pay')")
        @Operation(summary = "Resolve bill match exception", description = "Resolves a matching exception with action ACCEPT, VOID, or CORRECT", tags = {
                        "Vendor Bill API" })
        ...
        @Operation(summary = "Get vendor bill by id", description = "Retrieves a vendor bill by its unique identifier", tags = {
                        "Vendor Bill API" })
        ...
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:ap:view" })
        @PreAuthorize("hasAuthority('accounting:ap:view')")
        @Operation(summary = "Get vendor bill by origin event id", description = "Retrieves a vendor bill by origin goods-received event identifier", tags = {
                        "Vendor Bill API" })
        @ApiResponse(responseCode = "200", description = "Vendor bill found", content = @Content(schema = @Schema(implementation = VendorBillResponse.class)))
        @ApiResponse(responseCode = "404", description = "Vendor bill not found")
        ...
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:ap:view" })
        @PreAuthorize("hasAuthority('accounting:ap:view')")
        @Operation(summary = "List match candidates", description = "Lists unresolved match candidates for an ambiguous invoice event", tags = {
                        "Vendor Bill API" })
        ...
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:ap:pay" })
        @PreAuthorize("hasAuthority('accounting:ap:pay')")
        @Operation(summary = "Select match candidate", description = "Selects a candidate and approves corresponding vendor bill flow", tags = {
                        "Vendor Bill API" })
        ...
        public ResponseEntity<VendorBillResponse> selectMatchCandidate(
                ...
                VendorBillResponse response = vendorBillService.selectMatchCandidate(candidateId,
                                request.getOperatorId());
                return ResponseEntity.ok(response);
        }
