...
public class MappingKeyServiceImpl implements MappingKeyService {
    ...
    public MappingKeyResponse createMappingKey(@NonNull MappingKeyCreateRequest request) {
        ...
        if (log.isInfoEnabled()) {
            log.info(
                    "Creating mapping key: {} for category: {}",
                    maskText(request.getKeyName()),
        ...
        if (mappingKeyRepository.existsByPostingCategory_PostingCategoryIdAndKeyName(
            ...
            throw new ResponseStatusException(
                    ...
                    "Mapping key with name '" + trimmedName + "' already exists in posting category '"
        ...
        if (log.isInfoEnabled()) {
            log.info(
                    "Created mapping key: {} with ID: {}",
                    maskText(saved.getKeyName()),
    ...
    public MappingKeyResponse getMappingKey(@NonNull UUID mappingKeyId) {
        ...
        MappingKey mappingKey = mappingKeyRepository
                .findById(mappingKeyId)
                .orElseThrow(
                        () -> new ResponseStatusException(HttpStatus.NOT_FOUND, MAPPING_KEY_NOT_FOUND + mappingKeyId));
    ...
    @Override
    @Transactional
    public MappingKeyResponse updateMappingKey(@NonNull UUID mappingKeyId, @NonNull MappingKeyUpdateRequest request) {
        if (log.isInfoEnabled()) {
    ...
    public MappingKeyResponse deactivateMappingKey(@NonNull UUID mappingKeyId) {
        ...
        if (activeMappingCount > 0) {
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT,
                    "Mapping key '" + mappingKey.getKeyName() + "' has "
                            + activeMappingCount
