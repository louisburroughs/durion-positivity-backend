...
public class CollectionsAnalyticsReport {
    ...
    @Schema(
            description = "settled divided by invoiced, times 100, rounded HALF_UP to 2 decimals; null when invoiced"
                    ...
                    + " cash-only — stays at 0 for that same invoice, since no cash was applied to it. May be"
                    + " NEGATIVE when in-window reversals leave collected negative enough to outweigh"
                    + " nonCashSettled; it is not clamped.",
            example = "88.60",
            requiredMode = NOT_REQUIRED,
            nullable = true)
    @Nullable
