...
public interface DefaultGLMappingRepository extends JpaRepository<DefaultGLMapping, UUID> {
     ...
     */
    @Query("SELECT m FROM DefaultGLMapping m " + "WHERE m.eventType = :eventType "
            + "AND m.active = true "
    ...
    Optional<DefaultGLMapping> findActiveDefaultForEvent(
            @NonNull @Param("eventType") String eventType, @Nullable @Param("organizationId") UUID organizationId);
