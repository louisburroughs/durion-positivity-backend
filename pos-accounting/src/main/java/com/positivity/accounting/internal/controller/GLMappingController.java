...
public class GLMappingController {
         ...
         */
        @PostMapping
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:gl-mapping:create" })
        @PreAuthorize("hasAuthority('accounting:gl-mapping:create')")
        @EmitEvent(id = "ACCOUNTING_GL_MAPPING_CREATE", apiVersion = "1")
        @Operation(summary = "Create GL mapping", description = "Creates a new GL mapping for source-system external code resolution", tags = {
                        "GL Mapping API" })
         ...
         */
        @PostMapping("/resolve")
        @SecurityRequirement(name = "bearerAuth", scopes = { "accounting:gl-mapping:resolve" })
        @PreAuthorize("hasAuthority('accounting:gl-mapping:resolve')")
        @EmitEvent(id = "ACCOUNTING_GL_MAPPING_RESOLVE", apiVersion = "1")
        @Operation(summary = "Resolve GL mapping", description = "Resolves source-system external code to a GL account using effective-date rules", tags = {
                        "GL Mapping API" })
