#!/bin/bash
# Start Durion POS microservices from pre-built JARs with local profile (H2 databases)
...
java -jar pos-service-discovery/target/pos-service-discovery-*.jar \
  --spring.profiles.active=local > logs/pos-service-discovery.log 2>&1 &
...
java -jar pos-security-service/target/pos-security-service-*.jar \
  --spring.profiles.active=local > logs/pos-security-service.log 2>&1 &
...
java -jar pos-accounting/target/pos-accounting-*.jar \
  --spring.profiles.active=local > logs/pos-accounting.log 2>&1 &
