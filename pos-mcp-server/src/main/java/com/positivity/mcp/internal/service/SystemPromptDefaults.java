...
public final class SystemPromptDefaults {
    ...
    static final String GLOSSARY_LAYER_TEXT = """
            ...
            - `defined: false` — the glossary has no entry for that phrase. That is NOT by itself a reason to ask. Read the question again first: if it already names the measure — "busiest locations BY APPOINTMENT COUNT", "slowest suppliers BY LEAD TIME" — use the measure the question named and ANSWER, saying which you used.
            - Ask only when the phrase is undefined AND the question names no measure at all. Then ask rather than choosing one silently: a metric picked without being asked for reads as confident and cannot be checked, which is worse than a question.
            - Never ask because a date range was left unstated: that is what the date-window contract resolves. A missing required identifier remains a separate argument-validation question.
