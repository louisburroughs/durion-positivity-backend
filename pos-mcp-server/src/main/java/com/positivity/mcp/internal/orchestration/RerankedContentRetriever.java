...
final class RerankedContentRetriever implements ContentRetriever {
  ...
  private static @NonNull RankedContent rankContent(
    ...
    double phraseBoost = contentText.toLowerCase(Locale.ROOT).contains(queryText.toLowerCase(Locale.ROOT))
        ? 0.2
        : 0.0;
  ...
  private static @NonNull Set<String> tokens(@NonNull String text) {
    return Set.copyOf(List.of(normalize(text).toLowerCase(Locale.ROOT).split("[^a-z0-9]+"))
        .stream()
