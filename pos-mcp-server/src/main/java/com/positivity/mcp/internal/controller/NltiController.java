...
public class NltiController {
    ...
    @Operation(operationId = "submitNltiRequest", summary = "Submit a Natural Language Task Request", description = """
                    ...
                    Preconditions: a supplied sessionId is resolved within the caller's tenant; one owned by another \
                    subject of that tenant is rejected, one the tenant does not have (unknown, or another tenant's) \
                    starts a fresh session, and a session idle for more than 24 hours is silently replaced with a \
                    new one.
                    ...
                    Returns 202 with status ACCEPTED or an attached write-plan preview (with a fresh sessionId when \
                    the supplied one was absent from the caller's tenant), 403 when the supplied sessionId is owned \
                    by another subject of the same tenant, and 429 when the per-session rate limit (default 100 \
    ...
    @Operation(operationId = "confirmWritePlan", summary = "Confirm a Previewed Write Plan", description = """
                    ...
                    re-executing.
                    Returns 200 on execution or idempotent replay, 404 WRITE_PLAN_NOT_FOUND when no plan or \
                    request exists for the id, 404 SESSION_NOT_FOUND when the plan's session is absent from the \
                    caller's tenant, 403 when the plan's session belongs to another subject of that tenant or the \
                    tool permission is missing, 409 when the plan is cancelled, executing, previously failed, stale, \
                    or the idempotency key mismatches, 410 when the plan has expired, and 502 when the target tool \
                    call itself fails.
                    """)
    ...
    @Operation(operationId = "cancelWritePlan", summary = "Cancel a Previewed Write Plan", description = """
                    ...
                    outcome cancelled.
                    Returns 200 with the plan (idempotently for already-inactive plans), 404 WRITE_PLAN_NOT_FOUND \
                    when no plan or request exists for the id, 404 SESSION_NOT_FOUND when the plan's session is \
                    absent from the caller's tenant, 403 when the plan's session belongs to another subject of that \
                    tenant, and 409 when the plan is COMPLETE, EXECUTING or ERROR and can no longer be cancelled.
                    """)
