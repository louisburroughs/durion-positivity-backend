...
public class SystemPromptSeedRunner implements ApplicationRunner {
    ...
    private static @NonNull Map<String, String> buildSeedPrompts() {
        ...
        // #1613: the eight hardcoded role personas that used to be seeded here are gone. Personas are
        // an attribute of the role now, synced from pos-security-service by RolePersonaSyncRunner, so
        // a role created after a release gets its own persona without a Java edit — which is exactly
        // what seven roles carrying real permission grants never got.
        //
        // ROLE_USER is the one that stays: it is the fallback identity for a caller who resolves to no
        // eligible role, and it has no pos-security-service row to sync from.
        prompts.put(
                SystemPromptDefaults.ROLE_USER_PROMPT_NAME,
                RolePersonaRenderer.render(
                        "platform user",
