...
_On inventory: `inventory:availability:read` lets a technician ask whether a part can be promised — on-hand net of prior commitments (allocations, reservations, expired lots) plus the incoming/outgoing forecast — optionally scoped to one location. Since ADR-0057 (#1494) it is enforced by the availability endpoints; before that it was granted and enforced nowhere. TECHNICIAN is **not** granted `inventory:availability:search` (the per-location breakdown enumerating every location holding the SKU) nor any `inventory:on_hand:*` code (the stock record itself: counted quantity, lot and serial detail, location contents). Availability and on-hand are different questions and neither permission family implies the other._
