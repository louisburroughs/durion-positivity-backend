...
- **Two payload shapes (distinct):**
  1. *Telemetry* (`EventEmitted`/`EmittedEvent`, pos-event-receiver): `eventId`, `id`, `apiVersion`, `timestamp`, `elapsedMs`, `publishedAt`, and an optional `entityId` — latency/volume plus, for services that tag their `@EmitEvent` calls with an entity id, that entity id; NO actor or correlation id. Rolled up hourly (count, avg/p95/p99 elapsed).
...
- **Reconstruct "what happened to entity X":** pos-event-receiver serves time-window aggregates (`/v1/events/summary/lastHour|lastDay|lastWeek`) AND, since #1521, per-entity history via `GET /v1/events?entityId=&since=&page=&size=` — newest first, but only for events recorded with an entity id (many are not), and bounded to a 90-day lookback. For "who did what" rather than "what telemetry exists," the audit ledgers remain authoritative: `GET /v1/audit/events` (pos-security-service, filter by aggregateId/workorderId/productId/actorId/correlationId) and `GET /v1/nlt/audit` (pos-mcp-server, by correlationId/eventType/date).
