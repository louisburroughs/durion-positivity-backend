...
Verified pos-order operations (path → tool name `{firstPathSegment}_{operationId}`): `/v1/orders/carts` createCart → `orders_createcart`; addItem → `orders_additem`; cancelOrder → `orders_cancelorder`; `/v1/orders/price-overrides` applyPriceOverride → `orders_applypriceoverride`, approvePriceOverride → `orders_approvepriceoverride`, rejectPriceOverride → `orders_rejectpriceoverride`, getPendingApprovals → `orders_getpendingapprovals`. (Source: `pos-order/openapi.yaml`.)
