...
/** Unit tests for {@link ToolPermissionExceptionHandler}. */
class ToolPermissionExceptionHandlerTest {

    // ---------------------------------------------------------------
    // X-Correlation-Id header (ADR-0017 §4, #1729) — proves every
    // @ExceptionHandler method on ToolPermissionExceptionHandler sets the
    ...
    class XCorrelationIdHeader {
        ...
        /** One entry per {@code @ExceptionHandler} method on {@link ToolPermissionExceptionHandler}. */
        private static Stream<Named<HandlerInvocation>> handlerInvocations() {
            ...
            ToolPermissionExceptionHandler handler = new ToolPermissionExceptionHandler(clockProvider);
            ...
            return Stream.of(
                    ...
                    Named.of("handleNotFound", (HandlerInvocation)
                            request -> handler.handleNotFound(new NoSuchElementException("Tool not found"), request)));
        }
        ...
        void echoesInboundCorrelationId(HandlerInvocation invocation) {
            ...
            UUID inbound = UUID.fromString("00000000-0000-7000-8000-000000000141");
        ...
        void everyHandlerMethodIsCovered() {
            long handlerMethodCount = Arrays.stream(ToolPermissionExceptionHandler.class.getDeclaredMethods())
                    .filter(method -> method.isAnnotationPresent(ExceptionHandler.class))
                    .count();
            long methodSourceEntryCount = handlerInvocations().count();

            assertThat(methodSourceEntryCount)
                    .as("A new @ExceptionHandler method was added to ToolPermissionExceptionHandler without a "
                            + "matching entry in XCorrelationIdHeader#handlerInvocations() in "
                            + "ToolPermissionExceptionHandlerTest — add one so the X-Correlation-Id header "
