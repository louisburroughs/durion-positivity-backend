...
/**
 * #1180: compound questions (two information needs in one query) must not let chunks serving the
...
class RerankedContentRetrieverTest {
    ...
    private static Document permissionDoc(int n) {
        // Contains sub-query B verbatim; near-duplicates differing only in the trailing role name.
    ...
    /** Five near-duplicate permission chunks ranked 1-5, the format chunk fused at rank 6. */
    ...
    void compoundSlots_secondNeedBestChunkSurvivesCut() {
        ...
        // The dominant need's best chunk is still present — reservation adds, it does not evict all.
    ...
    void compoundQuery_bothNeedsAlreadyInTopK_unchanged() {
        // Format chunk fused at rank 1; permission chunks 2-6. Both needs' best chunks are in the
    ...
    void splitSubQueries_heuristics() {
        ...
        // Trailing fragments that are too short to be an information need suppress the split.
