...
--
-- Serving endpoint: E8 — GET /v1/accounting/analytics/vendor-spend (avgBillAmount), one call
--   per window. Budget (§6): 3. Tolerance (§2.1): avgBillAmount exact (scale-2 HALF_UP);
...
--
-- SEMANTICS — E8's avgBillAmount (q15 header): vendor_bill rows bucketed by bill_date
--   (naive window), REGARDLESS of status, avg = SUM(total_amount)/COUNT, scale 2 HALF_UP;
--   the model compares the two windows' avgBillAmount and keeps vendors whose increase
--   exceeds 10 %. Designed answer: V1 Evergreen alone (+12.0 %).
--
-- DIVERGENCE: avgBillAmount includes unpaid/rejected/voided bills (bill_date basis, no
...
WITH cur AS (
    SELECT b.vendor_id, COUNT(*) AS bill_count, ROUND(SUM(b.total_amount) / COUNT(*), 2) AS avg_bill
    FROM vendor_bill b
...
SELECT c.vendor_id,
       v.name,
       p.avg_bill                                    AS avg_bill_prior_year,
       c.avg_bill                                    AS avg_bill_current,
       ROUND(100 * (c.avg_bill - p.avg_bill) / NULLIF(p.avg_bill, 0), 2) AS yoy_increase_pct,
       (100 * (c.avg_bill - p.avg_bill) / NULLIF(p.avg_bill, 0)) > 10    AS exceeds_10_pct
FROM cur c
