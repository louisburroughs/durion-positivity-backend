...
| `spring.ai.ollama.chat.options.num-ctx`    | `OLLAMA_NUM_CTX` `32768`                    | Context window sent to Ollama as `num_ctx`. **Always set explicitly** (#1683) — left unset, Ollama applies the *host's* `OLLAMA_CONTEXT_LENGTH` (4096 by default) and silently drops the front of the context, i.e. the system prompt |
