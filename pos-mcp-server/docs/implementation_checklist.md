...
| 4 | **Shipped, now DORMANT** — router wired into both chat managers behind `mcp.model.tiering-enabled` (default flipped to false by #1683: both T2 tiers resolved to the same model, so the per-turn classification had one possible outcome; see gate4-tiered-router-design.md for the revival path), `TieredChatModelResolver`, tier-suffixed cache keys, shared T0 fast path, router telemetry; live routing-mix metrics still open | #1192 CLOSED (PR #1199, 2026-08-08) |
...
- Metrics filled: [x] (96/96 equivalence checks across 4 personas × blocking/streaming: tools, persona, workflow state, prompt layers all equal — values in the dated entry below) · Decision: **PASS** (approved Louis Burroughs / 2026-08-19) · Close-out tracked → #1214 (CLOSED 2026-08-19)
...
- Metrics filled: [x] · Decision: **PASS** (approved Louis Burroughs / 2026-08-19) · Close-out tracked → #1214 (CLOSED 2026-08-19)
- **2026-08-19 — the two deferred live items are now covered by the Wave 1 runs:** live fail-closed proven repeatedly (persona lacks-probes → 403 across four personas; write-gate fail-closed → 403/no-plan; admin fail-closed → 403; workflow ownership → 403), and migration-on-real-Postgres proven by the alpha stack booting through the full Flyway chain including V34 (Gate 2C activation depends on those seeded rows and passed 8/8). Permission behavior identical blocking vs streaming is the Gate 2A equivalence result (96/96).
...
- Metrics filled: [x] (workflow suite 8/8 — values in the dated entry below) · Decision: **PASS** (approved Louis Burroughs / 2026-08-19) · Close-out tracked → #1215 (CLOSED 2026-08-19)
...
- Metrics filled: [x] (router suite 7/7: simple-mix 100%/floor 80, tier→model table, p95 5653 ms/SLO 15000 — values in the dated entry below) · Decision: **PASS** (approved Louis Burroughs / 2026-08-19; the hardcoded `model.fallbackUsed` caveat is accepted as a signed exception, owned by #1397) · Close-out tracked → #1216 (CLOSED 2026-08-19)
...
- Metrics filled: [x] cutover eval green (recall@k 0.9216 vs floor 0.82; floors re-baselined 0.68/0.65/0.82 per #1179) · Decision: **PASS** (approved Louis Burroughs / 2026-08-19) · Close-out tracked → #1217 (CLOSED 2026-08-19; policy residue → #1396, RESOLVED same day)
- **2026-08-19 — #1396 resolved by tagging, verified live.** `security.guide` now carries `required-permissions: [security:permission:view, security:role:view]` in both config files (PR #1400); the deliberate content touch flipped the hash and forced a re-embed, and the post-deploy sweep confirms it: **RETRIEVAL LOCK OK, 39/39 hash-locked**, `security.guide` SKIPPED on the new hash with **15/15 chunks carrying required_permissions** (the 8 untagged chunks were replaced). Public-by-declaration set is now exactly the five operational guides, pinned by `RetrievalLockTest`. Operational notes from the rollout: the first two re-ingest attempts failed on the 30s `OLLAMA_EMBEDDING_TIMEOUT` while `ToolEmbeddingInitializer` saturated Ollama at startup — the alpha `.env` now sets 120s, and the QUEUED preload row reconciled to SKIPPED on the following restart as designed. _ev: /tmp/sweep3.log on the alpha host; ingestion job SUCCEEDED 2026-08-19T18:46:42Z._
- **2026-08-18 — retrieval-lock sweep executed on alpha (#1217, Wave 1 step 2): PASS.** `scripts/rag_lock_sweep.py` (read-only session) against `pos_user@localhost:5432/pos_mcp`: **39/39 documents hash-locked, 0 drifted, 0 orphaned, 163 embedded chunks, 0 NULL vectors**. SHA-256 of every `src/main/resources/rag/*.md` matches the newest LOADED/SKIPPED `mcp_rag_preload_record` row; no DB document is absent from the manifest. Permission coverage: 33/39 carry `required-permissions`; 6 are public by declaration (`accounting.de-bookkeeping`, `inventory.inv-cntrl`, `people.human-resources`, `shop.management`, `shop.management.guidelines`, `security.guide`). The offline half (deterministic/unique id, `rag-scope`, explicit permission decision, resolvable `source-path`, documented chunking) is asserted by `RetrievalLockTest` in the module surefire build. _ev: `pos-mcp-server/target/rag-lock-sweep/rag-lock-sweep.{md,json}` on the alpha host; command id `ae0d8082-6ea9-4fd7-b726-9accf2dc854f`._
  - ~~Open policy item (blocks Pass)~~ **RESOLVED 2026-08-19 (#1396, option: tag it):** `security.guide` was public at sweep time; it is now permission-bound (`security:permission:view` + `security:role:view`, PR #1400) and out of `PUBLIC_DOC_IDS` — live re-verification in the 2026-08-19 entry above.
...
- Metrics filled: [x] (write-gate live metrics in the dated 2026-08-19 entry below; the write-safety FIXTURE pass rate is exercised offline by the CI suites — the live full-flow run is the write-safety evidence of record) · Decision: **PASS** (approved Louis Burroughs / 2026-08-19; the two documented gaps — no submit-path telemetry, intent parser gate-unreachable for create/update — are accepted as signed exceptions, owned by #1397 and #1398) · Close-out tracked → #1218 (CLOSED 2026-08-19; gaps → #1397, #1398)
- **2026-08-20 — #1408 live-verified on alpha (image `sha-35b2f00`, harness/checkout at `35b2f007`): `wg-telemetry` and `wg-telemetry-outcome` PASS — the #1397 telemetry work is proven live.** No-`--allow-writes` run (harness mode `observe`, not the stricter `--read-only` flag; `--suite write-gate --write-target ...`, 00:47:10Z): 10 pass / 0 fail / 5 skipped — ACTION prompt → `PENDING_CONFIRMATION` plan preview, cancel 200 (idempotent), `wg-telemetry` PASS with `write.isWrite=true intentType=ACTION riskLevel=HIGH` joined **by correlation id** (0 unverified-attribution, no window fallback), `wg-telemetry-outcome` PASS with `write.confirmationOutcome=cancelled`. Full-flow run (`--allow-writes`, 00:47:15Z): 14 pass / 0 fail / 1 skipped (the cancel check skips by design after a confirm) — seeds 201/201, confirm → `COMPLETE`, idempotent replay, full audit chain, fail-closed 403, `wg-telemetry-outcome` PASS with `write.confirmationOutcome=confirmed`; `wg-intent-gap` PASSed in both runs (create-style prompt → `PENDING_CONFIRMATION` with planId — the #1398 signed exception evidence is flipped live too). Counter live in Prometheus: `nlt_write_plan_confirmation_count_total{outcome="cancelled"}=1` and `{outcome="confirmed"}=1` (registers on first increment — an empty result before any terminal outcome is not a defect). Gate 7 panels verified against the run's traffic after the dashboard resync: panel 27 renders the `confirmed`/`cancelled` outcome mix for the first time, panel 26 shows 11 write-capable telemetry records / 4 plans created, panel 25 carries the corroborating confirm-200 / cancel-200 / cancel-404 status series. Operational notes: `/opt/durion/alpha/nlti-write-target.json` was stale (pre-#1386 — seeds lacked `route`) and was refreshed from `scripts/fixtures/nlti-write-target.example.json` (backup `.bak-1408`); the harness requires `--write-target` only with `--allow-writes`, but a no-`--allow-writes` run should pass it anyway: without it there is no `clientContext.targetTool`, the submit stops at `NEEDS_CLARIFICATION`, and no plan (hence no terminal confirmation outcome) ever exists to observe. _ev: `nlti-live-verify-20260820T004710Z.{json,md}` and `nlti-live-verify-20260820T004715Z.{json,md}` under `pos-mcp-server/target/nlti-live-verify/` on the alpha eval checkout._
...
- 2026-08-19 — **#1397 shipped (offline): the NLTI pipeline now emits `nlti.request.telemetry`, closing the write-gate telemetry blind spot.** `NltiRequestServiceImpl.submit` emits one record per `POST /v1/nlt/requests` from a `finally` block (so rejected requests are on the stream too), carrying the caller's correlation id, `sessionId`/`requestId` (the chat path has neither), `routing.intentType`/`riskLevel`, `latency.totalMs`, `outcome`, and `write.isWrite=true` for every ACTION-classified request. `NltiWritePlanService` mirrors all five terminal confirmation outcomes (`confirmed`/`cancelled`/`expired`/`stale-data`/`superseded`) out of the `nlti_audit_event` table onto both `write.confirmationOutcome` and the new `nlt.write_plan.confirmation.count` counter (tagged `outcome`), so alert rules — which cannot query Loki — finally have a first-class signal instead of the HTTP-status proxy; `write.planArgsProvenance` carries per-provenance counts only, never argument names or values. Emission is guarded end to end: a failing emitter or an unresolvable actor degrades the record, never the request. Gate 7 panels 25/26/27 came off their substitute-query GAP notes (25 retained as the corroborating HTTP view — a confirm that passes the gate and fails downstream is `outcome=confirmed` with a 5xx, which only the status query sees). Harness: `wg-telemetry` SKIP → real assertion, plus a new `wg-telemetry-outcome` check (asserted after confirm on a live run, after cancel on a read-only one). `tools.rejectedPermissionCount`, `rag.retrieved` and `quality` stay unpopulated and are now documented as such on the schema record rather than left to mislead — tool permission gating runs in SQL, so the rejected set is never materialized in Java and `PermissionAwareMetadataFilter`'s DEBUG count is RAG documents, not tools, and must not be substituted. **Closed out 2026-08-20: the live rerun flipped `wg-telemetry` and `wg-telemetry-outcome` to PASS — see the dated 2026-08-20 entry (#1408).**
- 2026-08-19 — **#1398 shipped (offline): intent parser now routes create/update phrasings to the write gate.** `IntentParserServiceImpl` decouples intent type from risk: any write verb classifies as ACTION, with risk graded by verb class — destructive (delete/remove/"bulk " prefix) stays HIGH, mutating (create/update/change/modify/edit/adjust/set/cancel/reschedule/apply/transfer/receive) → MEDIUM, purely additive (add/register) → LOW. Whole-word matching (no "address"→"add" misfires); a write verb outranks query indicators so read-and-write prompts reach preview→confirm, never a silent read. `wg-intent-gap` reworked from FAIL-by-design to a real assertion (plan preview with a targetTool clientContext, `NEEDS_CLARIFICATION` missing=targetTool without one; plain ACCEPTED now FAILs); benchmark + unit tests extended (MEDIUM/LOW ACTION classes), write-safety fixtures +4 (create-po-reaches-write-gate, update-price-medium-risk, additive-write-low-risk, bulk-create-stays-high-risk). **Closed out 2026-08-20: the live rerun PASSed `wg-intent-gap` end to end (create prompt → plan preview with planId) — see the dated 2026-08-20 entry (#1408).**
...
- [x] Alerts for safety + quality regressions — _ev: PromQL rules have data (pos-mcp-server scrape job live); LogQL ruler added and verified 2026-08-20 (#1424 RESOLVED): 4 groups / 14 rules loaded, all health=ok_
- [x] Adaptive tuning moved disabled → shadow → controlled-live (post-approval) — _ev: shadow live on alpha (MCP_TUNING_MODE=shadow; 02:00Z cron fired 2026-08-20: "Computed 0 priority proposals ... mode=shadow"); controlled-live gate exercised 2026-08-22 (post-#1422 fix): live-mode cron downgraded to shadow on the absent baseline and wrote nothing — see the 2026-08-22 entry_
...
- [x] Alerts cover: failed-tool-call spike, permission-reject spike, fallback overuse, write-failure rate, confirmation-mismatch attempt, retrieval regression, latency SLO breach — _ev: rules defined in alerts/nlti-alerts.md; LogQL evaluation live since 2026-08-20 (#1424 RESOLVED — loki ruler, 4 groups / 14 rules, health=ok); PromQL alerts have data_
...
- [x] Tuning promotion requires eval improvement or approved neutral — _ev: eval-gate plumbing verified live (/opt/eval mount present, MCP_TUNING_EVAL_RESULT_PATH + 48h freshness in-container); gate logic covered by 18 tuning unit tests AND exercised live 2026-08-22: live-mode cron logged "downgraded to shadow: eval baseline not found", wrote nothing_
...
- [x] Live tuning cannot promote if eval thresholds fail — _ev: evalGateAllowsLive fails closed on absent/stale/failed baseline (unit-tested); proven live 2026-08-22: with the baseline deliberately absent, the live-mode cron downgraded to shadow and `mcp_tool.priority` was untouched_
...
- [x] Verified: adaptive tuning cannot silently mutate priorities — _ev: shadow-only writes nothing (live log line + code path); live path gated on eval file; proven with a real proposal in-flight 2026-08-21/22 (#1422 fixed): shadow computed 1.0→0.9688 without writing, and the live-mode night wrote nothing behind the eval gate_
...
- Metrics filled: [x] · Decision: **PASS** (approved Louis Burroughs / 2026-08-20; signed at approval with two exceptions — #1422 adaptive-tuning proposals structurally 0, #1424 LogQL rules not evaluating — **both since retired on live evidence**: #1424 on 2026-08-20 (loki ruler live), #1422 on 2026-08-22 (real shadow proposal + live-gate downgrade night); see the dated 2026-08-22 entry) · Close-out tracked → #1219
- **2026-08-22 — #1422 signed exception RETIRED: adaptive tuning proven end to end on live data.** The #1422 fix (PR #1427, deployed 2026-08-20) attributes a real `tool_id` to every tool execution; ten `--suite write-gate --allow-writes` runs put `price_deletepromotioneligibilityrule` at the tuner's ≥10-invocation threshold. **Shadow half (2026-08-21 02:00Z cron):** first-ever real proposal — `Computed 1 priority proposals without writing (effective mode=shadow)`, one `mcp.tuning.shadow` JSON line (current 1.0 → proposed 0.9688, delta −0.0312, success_rate 1.0), `mcp_tuning_proposals{mode="shadow"}=1`; `admin-tuning-shadow` flipped PASS. **Live-gate half (2026-08-22 02:00Z cron):** `MCP_TUNING_MODE=live` for one night with the eval baseline deliberately absent — the cron logged `Live tuning downgraded to shadow: eval baseline not found` and wrote nothing (`mcp_tool.priority` unchanged at 1.0), proving the gate refuses promotion without a fresh passing eval; `admin-tuning-gate` flipped PASS. Admin suite final: **7 PASS / 0 FAIL / 1 SKIP** (the skip is the optional `admin-mutation` probe). Alpha reverted to `MCP_TUNING_MODE=shadow` immediately after. The #1424 exception was retired 2026-08-20 the same day it was signed (loki ruler added: 4 groups / 14 rules, all health=ok). Both Gate 7 signed exceptions are now retired on live evidence; the PASS decision stands without qualification. _ev: `nlti-live-verify-20260821T124910Z.{json,md}` and `nlti-live-verify-20260822T070543Z.{json,md}` on the alpha eval checkout; PRs #1427, #1425._
