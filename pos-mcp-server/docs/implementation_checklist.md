...
- [x] Arguments schema-validated before proxy call — _ev: `OpenApiToolProvider.buildParameterSchema` builds a `JsonObjectSchema` envelope with **typed + required path params** (from the path template) and **typed + required query params** (from `input_schema`, which the mapper now persists from `operation.getParameters()`; type mapped string/integer/number/boolean). langchain4j validates both. Only headers/body remain free-form objects._
