...
class StorageLocationContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void getStorageLocations_filterByType_returns200() throws Exception {
        Page<StorageLocationResponse> page = new PageImpl<>(List.of(response(
                UUID.fromString("00000000-0000-0000-0000-000000000001"), "Bin-A", StorageLocationType.BIN, "ACTIVE")));

        when(storageLocationService.listStorageLocations(eq(SITE_ID), eq(StorageLocationType.BIN), any(), any()))
                .thenReturn(page);

        mockMvc.perform(withGatewayAuth(
                        get("/v1/locations/{siteId}/storage-locations", SITE_ID).param("type", "BIN")))
                .andExpect(status().isOk());
    }
    ...
    void getStorageLocation_whenFound_returns200() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(get("/v1/locations/{siteId}/storage-locations/{id}", SITE_ID, STORAGE_ID)))
                .andExpect(status().isOk())
    ...
    void getStorageLocation_whenNotFound_returns404() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(get("/v1/locations/{siteId}/storage-locations/{id}", SITE_ID, missingId)))
    ...
    void patchStorageLocation_validUpdate_returns200() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(patch("/v1/locations/{siteId}/storage-locations/{id}", SITE_ID, STORAGE_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                                                { "name": "Bin-A1-Renamed" }
                                                                """)))
     ...
     */
    private StorageLocationResponse response(UUID id, String name, StorageLocationType type, String status) {
