...
class ContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void testGetLocation_HappyPath() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(get("/v1/locations/{id}", locationId).header("X-Correlation-Id", "cp-002-get")))
    ...
    void testGetAllLocations_returnsArray() throws Exception {
        mockMvc.perform(withGatewayAuth(get("/v1/locations").header("X-Correlation-Id", "cp-004")))
                .andExpect(status().isOk())
    ...
    void testDeleteLocation_returns204() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(
                        delete("/v1/locations/{id}", locationId).header("X-Correlation-Id", "cp-005-delete")))
    ...
    void testLocationValidation_existing_returnsExistsAndActive() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(
                        get("/v1/locations/{id}/validation", locationId).header("X-Correlation-Id", "cp-006-validate")))
                .andExpect(status().isOk())
