...
/**
 * Retry and dead-letter handling for the module's Kafka listeners (ADR-0044 §4, pos-inventory
 * {@code KafkaErrorHandlingConfig} pattern).
 *
 * <p>Failed records are retried with exponential backoff; when retries are exhausted the record
 * is published to {@code {topic}.dlq} so poison messages surface for alerting instead of
 * silently blocking or dropping. The consumers are idempotent ({@code processed_events}), so
 * the retries are harmless.
 *
 * <p>Spring Boot wires a single {@code CommonErrorHandler} bean into the auto-configured
 * listener container factory, so declaring the bean is sufficient.
 */
@Slf4j
@Configuration
