...
public class LocationHierarchyService implements LocationAncestorResolver {
    ...
    /**
     * Expands a caller's {@link Reach} once per request: the union of the inclusive descendant set
     * of every assigned node on every dimension the permission is scoped on. A permission scoped on
     * both dimensions is satisfied by either rollup, so both are unioned (ADR-0061 §2).
     *
