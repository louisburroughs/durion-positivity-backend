...
class PerformanceValidationIntegrationTest {
    ...
    private BaseContext createLightweightContext(int index) {
    ...
    private BaseContext createUserContext(int userId, int requestIndex) {
        if (requestIndex % 3 == 0) {
            ...
            return TestingContext.builder()
                    .testingFramework("junit5")
                    .testTypes(List.of("unit", "integration"))
                    .build();
        }
    }

    private BaseContext createComplexContext(int index) {
        return ArchitectureContext.builder()
                .serviceType("microservice")
                .domain("complex-domain-" + index)
                .scalabilityRequirements("high")
                .integrationPatterns(List.of("event-driven", "api-gateway", "circuit-breaker"))
                .build();
    }
}
