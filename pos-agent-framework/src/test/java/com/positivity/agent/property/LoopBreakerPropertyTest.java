...
import com.positivity.agent.core.LoopBreakerMixin;
...
public class LoopBreakerPropertyTest {
    ...
    void recurringPatternDetection(@ForAll @IntRange(min = 3, max = 10) int patternLength) {
        ...
        // Should detect pattern and escalate before max iterations
        Assume.that("ESCALATION".equals(response.getStatus()));
        Assume.that(testAgent.getIterationCount() < 50);
        Assume.that(response.getOutput().contains("recurring pattern detected"));
    }
    ...
    void escalationTriggering(@ForAll @IntRange(min = 1, max = 15) int maxIterations) {
        ...
        if (testAgent.getIterationCount() >= maxIterations) {
            Assume.that("ESCALATION".equals(response.getStatus()));
            Assume.that(response.getOutput().contains("maximum iterations exceeded"));
    ...
    void loopBreakerStateReset(@ForAll @IntRange(min = 1, max = 10) int firstRun,
        ...
        testAgent.processRequest(request1);
        ...
        Assume.that("SUCCESS".equals(response2.getStatus()));
        Assume.that(testAgent.getIterationCount() == secondRun);
    }
