...
public class RequirementsAnalyzer {
    ...
    private List<String> identifyActors(String body, List<ParsedIssue.Section> sections) {
        ...
        if (lowerText.contains("customer"))
            actors.add("Customer");
        if (lowerText.contains("developer"))
            actors.add("Developer");
        if (lowerText.contains("system"))
            actors.add("System");
    ...
    private List<Requirement> extractRequirementsFromText(String text, EarsPattern defaultPattern) {
        ...
        for (String line : lines) {
            ...
            if (!line.isEmpty()) {
                // For explicit patterns (UNWANTED, STATE_DRIVEN), use them unless text clearly
                // indicates otherwise
                EarsPattern pattern;
    ...
    private EarsPattern determinePattern(String text) {
        ...
        if (lower.contains("when ") || lower.contains("if ")) {
            return lower.contains("error") || lower.contains("fail") ? EarsPattern.UNWANTED : EarsPattern.EVENT_DRIVEN;
        }
