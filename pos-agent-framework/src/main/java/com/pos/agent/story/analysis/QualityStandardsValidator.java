...
public class QualityStandardsValidator {
     ...
     * Requirement 7.2: Ensure every rule has acceptance criteria to ensure
     * verifiability
     * 
     ...
     * Requirement 7.3: Ensure actor, trigger, and outcome are present to ensure
     * completeness
     * 
     * @param text The requirements text to check
    ...
    public List<String> checkRequirementCompleteness(String text) {
        ...
        // Split into individual requirements (lines starting with SHALL or Gherkin
        // keywords)
        ...
        for (String line : lines) {
            ...
            if (EARS_PATTERN.matcher(trimmed).find()) {
                ...
                // For EARS, "SHALL" is the outcome indicator, so if EARS pattern matches,
                // outcome is present
                ...
                if (!hasActor || !hasTrigger) {
                    ...
                    incomplete.add(
                            "Incomplete requirement (missing " + String.join(", ", missing) + "): \"" + preview + "\"");
                }
                // Skip Gherkin check for EARS requirements
                continue;
            }

            // Check Gherkin-style requirements - only for lines that start with Gherkin
            // keywords
            ...
            if ((trimmed.toLowerCase().startsWith("given") ||
                ...
                // Gherkin scenarios should have Given (actor/state), When (trigger), Then
                // (outcome)
                ...
                if (!hasGiven || !hasWhen || !hasThen) {
                    ...
                    String preview = trimmed.length() > 80 ? trimmed.substring(0, 80) + "..." : trimmed;
                    incomplete.add("Incomplete Gherkin scenario (missing " + String.join(", ", missing) + "): \""
                            + preview + "\"");
                }
            }
        }
     ...
     * Requirement 7.6: If a requirement cannot be made verifiable, flag it as an
     * open question
     * 
     * @param text The requirements text to check
        ...
        for (String line : lines) {
            ...
            if (EARS_PATTERN.matcher(trimmed).find() || GHERKIN_PATTERN.matcher(trimmed).find()) {
                ...
                if (matcher.find()) {
                    String preview = trimmed.length() > 80 ? trimmed.substring(0, 80) + "..." : trimmed;
                    unverifiable
                            .add("Unverifiable requirement (contains '" + matcher.group() + "'): \"" + preview + "\"");
                }
            }
        }
