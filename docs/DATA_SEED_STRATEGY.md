...
| pos-catalog `R__seed_reference_catalog*.sql` (5 files) | 2* | **In progress** — the 500 products converted to `scripts/fixtures/seed/alpha/catalog/`, now including Wave-2 fields (manufacturer name/brand, country, type — manufacturer data travels on the product fact for warranty/supplier replicas). File 2 deletes at the §5.4 reseed. Remaining: categories, services, pricing, product_uom have no ingest path (interim: Flyway + services `facts/replay`); category-by-name resolution is the open follow-up wave |
