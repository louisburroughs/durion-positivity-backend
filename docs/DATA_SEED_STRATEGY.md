...
| pos-catalog `R__seed_reference_catalog*.sql` (5 files) | 2* | **In progress** — the 500 products converted to `scripts/fixtures/seed/alpha/catalog/` (facts publish per row; no post-seed replay needed for pipeline-loaded products). Remaining: categories, services, pricing, product_uom have no ingest path (interim: Flyway + services `facts/replay`), and the Wave-1 ingest record drops manufacturer/country/type — a follow-up bulk-ingest wave is needed before file 2 can be deleted at the §5.4 reseed |
| pos-shop-manager `R__seed_shop_manager_mechanics.sql` | 2 | **Audited: hand-seeded projection whose feed was never wired.** The feed now exists — `PeopleEventsListener` drives `MechanicSyncService` from TECHNICIAN staffing assignments and terminal employee statuses, so the people pack populates mechanics via Kafka. The mechanic rows in this file delete with the verified alpha reseed (§5.4); the `mechanic_skill` rows are genuine shop-manager data (proficiency/ASE certs live nowhere else) and need their own fixture or a decision to stay seeded |
