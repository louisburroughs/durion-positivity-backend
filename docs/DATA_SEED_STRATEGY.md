...
| pos-people-contact `R__seed_people_contact_operational_data.sql` | 2 | **Audited: superseded by the pipeline for identities.** Employee persons arrive via `createEmployee`'s `requestPersonUpsert` command, customer/commercial-contact persons via the customer packs' `createPerson` path — no fixture needed. The `user_person_links` section remains coupled to the unconverted security users; the file deletes with them at the §5.4 reseed |
...
| pos-vehicle-inventory `R__seed_vehicle_inventory_operational_data.sql` | 2 | **Converted** — `scripts/fixtures/seed/alpha/vehicle/vehicles.csv` with stable owner keys; the driver resolves owners to live partyIds at run time (customers must load first); deletion waits on the verified alpha reseed (§5.4) |
| pos-catalog `R__seed_reference_catalog*.sql` (files 1–6) | 2* | **In progress** — the 500 products converted to `scripts/fixtures/seed/alpha/catalog/`, now including Wave-2 fields (manufacturer name/brand, country, type — manufacturer data travels on the product fact for warranty/supplier replicas). File 2 deletes at the §5.4 reseed. Remaining: categories, the 50 general services, pricing and product_uom have no ingest path (interim: Flyway + services `facts/replay`); category-by-name resolution is the open follow-up wave |
