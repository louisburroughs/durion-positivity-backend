...
- **No container on the box** — the service was added by the commit being synced, and its image is
  ...
  also creates the service's `durion/<service>` ECR repository when it is missing (nothing else
  creates it: this repo has no infrastructure-as-code for the AWS side), so a new service needs no
  manual AWS step before its first push.
