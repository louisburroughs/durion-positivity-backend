...
- 2026-08-07: pos-order previously declared `spring.application.name: order` but carried no Eureka client dependency, so it never actually registered and `lb://ORDER` could not resolve. The closure wave added the dependency and eureka client config; the registry row above is now accurate at runtime. pos-order, pos-invoice, pos-accounting, and pos-bulk-loader runtime clients now follow the `service-id` + `@LoadBalanced` pattern (see open-items-plan status note).
