...
/**
 * Drains {@code event_outbox} to Kafka (ADR-0044 §4, PRD §9.3 — topic {@code marketing.events.v1}).
 *
...
public class OutboxPublisher {
    ...
    @Value("${pos.marketing.outbox.send-timeout-ms:10000}")
    ...
    public OutboxPublisher(
        ...
        this.publishedCounter = registry == null
                ? null
                : Counter.builder("marketing.outbox.published")
                        .description("Outbox events successfully published to Kafka")
                        .register(registry);
        this.failedCounter = registry == null
                ? null
                : Counter.builder("marketing.outbox.publish.failures")
                        .description("Outbox publish attempts that failed")
                        .register(registry);
    }

    @Scheduled(fixedDelayString = "${pos.marketing.outbox.poll-interval-ms:1000}")
