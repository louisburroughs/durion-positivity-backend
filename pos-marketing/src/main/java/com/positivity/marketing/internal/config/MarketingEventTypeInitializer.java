...
@Component
public class MarketingEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(MarketingEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;
    private final String apiSecret;

    public MarketingEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-marketing");
    ...
    public void run(ApplicationArguments args) {
        log.info(
                "Registering {} marketing event types",
                MarketingEventTypes.all().size());
        initializerSupport.registerEventTypes(MarketingEventTypes.all(), this::registerEventType);
    }
    ...
    private void registerEventType(EventTypeRegistration registration) {
        try {
            ...
            log.warn("Failed to register marketing event type {}: {}", registration.getTypeCode(), ex.getMessage());
        }
    }
}
