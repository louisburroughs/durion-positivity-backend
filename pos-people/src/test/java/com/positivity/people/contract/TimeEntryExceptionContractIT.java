...
class TimeEntryExceptionContractIT extends BaseContractIntegrationTest {
	...
	void CP_120_030_createException_returns200() throws Exception {
		...
		mockMvc
			.perform(withAuth(post("/v1/people/exceptions").contentType(MediaType.APPLICATION_JSON).content(payload)))
	...
	void CP_120_034_listExceptions_returns200() throws Exception {
		mockMvc.perform(withAuth(get("/v1/people/exceptions").accept(MediaType.APPLICATION_JSON)))
			.andExpect(status().isOk())
	...
	void CP_120_034_listExceptions_byEmployee_returns200() throws Exception {
		mockMvc
			.perform(withAuth(
					get("/v1/people/exceptions").param("employeeId", "EMP-001").accept(MediaType.APPLICATION_JSON)))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$").isArray());
	}
	...
	void VE_120_030_waiveException_missingReason_returns400() throws Exception {
		mockMvc
			.perform(withAuth(
					post("/v1/people/exceptions/{id}/waive", UUID.fromString("00000000-0000-0000-0000-000000000000"))
