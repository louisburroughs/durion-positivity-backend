 ...
 * PostgresSliceTestBase} subclass.
 *
 * <p>A listener rather than a {@code @BeforeEach}, for two reasons a JUnit callback cannot meet:
 *
 * <ul>
 *   <li><strong>It has to happen before the transaction.</strong> {@code @DataJpaTest} opens the
 *       test's transaction before any {@code @BeforeEach} runs, and {@code TenantAwareDataSource}
 *       binds the tenant onto a connection at checkout — so a tenant bound afterwards never reaches
 *       the connection the whole test then uses, and row-level security rejects every write. This
 ...
 * rolled back and after every {@code @AfterEach}.
 */
