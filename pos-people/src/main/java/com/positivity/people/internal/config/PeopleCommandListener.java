...
/**
 * Kafka command listener for {@code people.commands.v1} (ADR-0044 §4, issue #875).
 *
 * <p>Supported command types:
 * <ul>
 * <li>{@code people.outbox.replay-requested} — consumer-initiated drift repair and replica
...
public class PeopleCommandListener {

    private static final String PAYLOAD = "payload";

    /** Canonical dotted name normalized to command-type form: PEOPLE_OUTBOX_REPLAY_REQUESTED. */
    private static final String COMMAND_OUTBOX_REPLAY_REQUESTED = "PEOPLE_OUTBOX_REPLAY_REQUESTED";
    ...
    @Value("${pos.people.outbox.replay.max-lookback:P30D}")
    ...
    @KafkaListener(
            topics = "${pos.people.kafka.commands-topic:people.commands.v1}",
            groupId = "${pos.people.kafka.commands-consumer-group:pos-people-commands}")
    public void onCommand(@NonNull String message) {
        try {
            ...
            // Normalize dotted command names (people.outbox.replay-requested) to one form.
            String commandType =
