...
public class OutboxPublisher {
    ...
    @Value("${pos.people.outbox.send-timeout-ms:10000}")
    ...
    public OutboxPublisher(
        ...
        this.publishedCounter = registry == null
                ? null
                : Counter.builder("people.outbox.published")
                        .description("Outbox events successfully published to Kafka")
                        .register(registry);
        this.failedCounter = registry == null
                ? null
                : Counter.builder("people.outbox.publish.failures")
                        .description("Outbox publish attempts that failed")
                        .register(registry);
    }

    @Scheduled(fixedDelayString = "${pos.people.outbox.poll-interval-ms:1000}")
