...
public class AttendanceDiscrepancyReportResponse {
    ...
    @Schema(description = "Total attendance time in decimal hours", example = "8.00",
            requiredMode = Schema.RequiredMode.REQUIRED)
    private double totalAttendanceHours;

    @Schema(description = "Total approved job time in decimal hours", example = "6.00",
            requiredMode = Schema.RequiredMode.REQUIRED)
    private double totalJobHours;

    @Schema(description = "Attendance minus job time in decimal hours", example = "2.00",
            requiredMode = Schema.RequiredMode.REQUIRED)
    private double discrepancyHours;

    @JsonProperty("isFlagged")
    @Schema(description = "True when absolute discrepancy exceeds threshold", example = "true",
            requiredMode = Schema.RequiredMode.REQUIRED)
    private boolean flagged;

    @Schema(description = "Threshold in minutes used for this row", example = "60",
            requiredMode = Schema.RequiredMode.REQUIRED)
