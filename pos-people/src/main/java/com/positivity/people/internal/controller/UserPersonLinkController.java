...
public class UserPersonLinkController {
        ...
        public ResponseEntity<UserPersonLinkResponse> linkUserToPerson(
                ...
                boolean alreadyLinked = linkService.linkExistsByUserIdAndPersonId(request.getUserId(),
                                request.getPersonId());
        ...
        public ResponseEntity<UserPersonLinkResponse> createUserPersonLink(
                ...
                boolean alreadyLinked = linkService.linkExistsByUserIdAndPersonId(request.getUserId(),
                                request.getPersonId());
                UserPersonLinkResponse response = linkService.createUserLink(request.getUserId(),
                                request.getPersonId());
                return alreadyLinked
                                ? ResponseEntity.ok(response)
                                : ResponseEntity.status(HttpStatus.CREATED).body(response);
