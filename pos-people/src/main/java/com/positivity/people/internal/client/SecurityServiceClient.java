...
public class SecurityServiceClient {
    ...
    @NonNull public Optional<User> getUserByUsername(@NonNull String username) {
     ...
     */
    @NonNull private Role getRoleByName(@NonNull String roleName) {
    ...
    @NonNull public List<RoleDto> getAvailableRoles(@NonNull String scope) {
        log.debug("Fetching available roles for scope: {}", scope);

        List<RoleDto> roles = restClient.get()
            .uri(uriBuilder -> uriBuilder.path("/v1/roles").queryParam("scopeType", scope).build())
            .retrieve()
            .onStatus(statusCode -> statusCode.value() == 400, (request, response) -> {
                throw new IllegalArgumentException("Invalid scope for role lookup: " + scope);
            })
            .onStatus(statusCode -> statusCode.value() == 404, (request, response) -> {
                throw new jakarta.persistence.EntityNotFoundException("Roles endpoint not found in security service");
            })
            .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                int statusCode = response.getStatusCode().value();
    ...
    @NonNull public List<UserRoleDto> getUserRoleAssignments(@NonNull UUID userId, Boolean includeHistory,
            LocalDateTime endDate) {
        log.debug("Fetching role assignments for userId: {}, includeHistory: {}, endDate: {}", userId, includeHistory,
                endDate);

        List<UserRoleDto> assignments = restClient.get()
            .uri(uriBuilder -> uriBuilder.path("/v1/roles/assignments/user/{userId}")
                .queryParam("includeHistory", includeHistory)
                .queryParam("endDate", endDate)
                .build(userId))
            .retrieve()
            .onStatus(statusCode -> statusCode.value() == 400, (request, response) -> {
                throw new IllegalArgumentException("Invalid request while listing assignments for userId: " + userId);
            })
            .onStatus(statusCode -> statusCode.value() == 404, (request, response) -> {
                throw new jakarta.persistence.EntityNotFoundException(
            ...
            .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                int statusCode = response.getStatusCode().value();
                throw new SecurityServiceException("Security service failed while listing role assignments",
                        statusCode);
            })
    ...
    @NonNull public UserRoleDto assignRole(@NonNull UserRoleAssignmentRequest request) {
        ...
        RoleAssignmentRequest apiRequest = new RoleAssignmentRequest(userIdUuid, role.getId(),
                scopeLocationIds != null ? ScopeType.LOCATION : ScopeType.GLOBAL, scopeLocationIds,
        ...
        RoleAssignment assignment = restClient.post()
            ...
            .onStatus(statusCode -> statusCode.value() == 404, (httpRequest, httpResponse) -> {
                throw new jakarta.persistence.EntityNotFoundException("User or role not found for assignment request");
            })
    ...
    public void revokeRole(@NonNull UUID userId, @NonNull String roleCode, LocalDateTime endDate) {
        ...
        restClient.delete()
            .uri(uriBuilder -> uriBuilder.path("/v1/roles/assignments/{assignmentId}")
            ...
            .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                int statusCode = response.getStatusCode().value();
                throw new SecurityServiceException("Security service failed while revoking role assignment",
                        statusCode);
            })
            .toBodilessEntity();
    }
    ...
    private UserRoleDto mapToUserRoleDto(RoleAssignment assignment, String roleCode) {
        ...
        if (assignment.getScopeType() == ScopeType.LOCATION && assignment.getScopeLocationIds() != null
            ...
            locationId = assignment.getScopeLocationIds()
                .stream()
                .findFirst()
