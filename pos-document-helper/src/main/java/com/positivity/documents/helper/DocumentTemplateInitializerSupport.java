...
public class DocumentTemplateInitializerSupport {
    ...
    public int registerTemplates(
        ...
        if (failCount.get() > 0) {
            log.warn(
                    "[{}] Template registration completed with {} failures out of {} attempts",
                    serviceName,
                    failCount.get(),
                    templates.size());
        }
        ...
        if (successCount.get() == 0) {
            String errorMsg =
                    String.format("All %d template registrations failed for service %s", templates.size(), serviceName);
            throw new TemplateRegistrationException("ALL_TEMPLATES", errorMsg);
        }

        log.info(
                "[{}] Successfully registered {}/{} document templates",
                serviceName,
                successCount.get(),
                templates.size());
