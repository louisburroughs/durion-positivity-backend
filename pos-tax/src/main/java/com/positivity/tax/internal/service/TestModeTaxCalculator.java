 ...
 * percentage-based tax calculations using rates configured in application properties.
 */
@Slf4j
@Component
public class TestModeTaxCalculator {
    ...
    private List<TaxJurisdiction> determineJurisdictions(@NonNull TaxCalculationRequest request) {
        ...
        BigDecimal stateRate = defaultRates.getOrDefault("STATE", BigDecimal.ZERO);
        if (stateRate.compareTo(BigDecimal.ZERO) > 0) {
            ...
            jurisdictions.add(TaxJurisdiction.builder()
                .jurisdictionCode(request.getStateCode() + "-STATE")
                .jurisdictionName(request.getStateCode() + " State Tax")
        ...
        BigDecimal countyRate = defaultRates.getOrDefault("COUNTY", BigDecimal.ZERO);
        if (countyRate.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal countyTax = taxBase.multiply(countyRate)
                .setScale(2, RoundingMode.HALF_UP);
            jurisdictions.add(TaxJurisdiction.builder()
                .jurisdictionCode("COUNTY-" + request.getPostalCode())
                .jurisdictionName("County Tax")
        ...
        BigDecimal cityRate = defaultRates.getOrDefault("CITY", BigDecimal.ZERO);
        if (cityRate.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal cityTax = taxBase.multiply(cityRate)
                .setScale(2, RoundingMode.HALF_UP);
            jurisdictions.add(TaxJurisdiction.builder()
                .jurisdictionCode("CITY-" + request.getCity())
                .jurisdictionName(request.getCity() + " City Tax")
