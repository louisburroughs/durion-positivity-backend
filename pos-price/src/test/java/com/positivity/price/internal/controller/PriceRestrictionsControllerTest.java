...
import com.positivity.price.internal.service.RestrictionEvaluationService;
import com.positivity.price.internal.service.RestrictionOverrideService;
...
class PriceRestrictionsControllerTest extends BaseContractIntegrationTest {
     ...
     *
     * <p>
     * This test may already pass in RED because
     * {@code @PreAuthorize("isAuthenticated()")}
     * is present on the controller method and Spring Security enforces it.
     * </p>
     *
    ...
    /**
     * RRC-002: authenticated request with a valid body, service returns an ALLOW
     * decision,
    ...
    void RRC002_authenticated_validRequest_serviceReturnsAllow_returns200() throws Exception {
        ...
        when(evaluationService.evaluate(any()))
                .thenReturn(List.of(
                        new RestrictionEvaluationResult(productId, RestrictionDecision.ALLOW, List.of(), List.of())));
    ...
    /**
     * RRC-003: authenticated request, service returns BLOCK decision, must respond
     * 200
     ...
     * return 200 with {@code decision == "RESTRICTION_UNKNOWN"} (no exception
     * thrown).
     *
     ...
     * {@link com.positivity.price.internal.dto.RestrictionEvaluationRequest#items()}).
     *
     * Issue: #43 (ADR-0017 §400)
     */
    @Test
    ...
    /**
     * RRC-007: authenticated user lacking {@code pricing:restriction:override}
     * authority
    ...
    /**
     * RRC-008: authenticated user with {@code pricing:restriction:override}
     * authority and
    ...
    void RRC008_hasOverrideAuthority_validBody_returns200WithOverrideId() throws Exception {
        ...
        when(overrideService.createOverride(any()))
                .thenReturn(new RestrictionOverrideResponse(
                        overrideId, Instant.now().plus(24, ChronoUnit.HOURS)));
