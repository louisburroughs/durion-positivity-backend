 ...
 * {@link PricePostgresContainer}, with Hibernate {@code validate} and the pool connected as the
 * non-owner {@code pos_app} role — the arrangement alpha runs (ADR-0062 §3).
 *
 * <p>It exists because the dialect is part of what a repository test is testing. An all-optional
 * JPQL filter written as {@code (:param IS NULL OR column = :param)} passes on H2 and can be
 * rejected by PostgreSQL at parse time, for every call, when the placeholder's type cannot be
 * inferred (issue #1891); only a test that issues the statement to PostgreSQL can see that.
 *
 ...
 *   <li>{@link SliceSupportConfig}: JPA auditing and the {@link java.time.Clock} it reads
...
public abstract class PostgresSliceTestBase {
    ...
    static void tenantAwareDatasource(DynamicPropertyRegistry registry) {
        PricePostgresContainer.registerDataSourceProperties(registry);
