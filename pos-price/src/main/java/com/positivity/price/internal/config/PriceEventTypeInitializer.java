...
/**
 * Initializes all event types for the pos-price module on application startup.
 * Uses the pos-events upsert endpoint for idempotent registration.
 */
@Component
public class PriceEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(PriceEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;
    private final String apiSecret;

    public PriceEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-price");
        this.apiSecret = apiSecret;
    }

    @Override
    public void run(ApplicationArguments args) {
        log.info("Registering {} price event types", PriceEventTypes.all().size());

        initializerSupport.registerEventTypes(
                PriceEventTypes.all(),
                this::registerEventType);

        log.info("Price event type registration complete");
    }
