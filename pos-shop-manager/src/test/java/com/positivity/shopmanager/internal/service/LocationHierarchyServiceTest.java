...
/**
 * Materialised location-scope ancestor sets on the pos-shop-manager {@code ext_location} replica
 * (ADR-0061 §2, #1872), driven end to end through {@link LocationEventsListener} against a real
 * database (H2 via the {@code test} profile, as this module's other repository tests do) so the
 * edge replacement, the derived traversal queries, the {@code text} column converter and the
 * subtree recompute are all exercised together — the same proof pos-people carries for its copy. The closure itself is pinned in pos-domain-events'
 * {@code LocationAncestryTest}; what matters here is that the module feeds it the right rows and
 * writes the result back where {@link LocationHierarchyService#ancestorsOf} reads it.
 */
@SpringBootTest
@ActiveProfiles("test")
@Transactional
@DisplayName("pos-shop-manager LocationHierarchyService — materialised scope ancestor sets")
class LocationHierarchyServiceTest {
    ...
    private void locationFact(UUID locationId, long version, String... edges) {
        ...
        listener.onLocationEvent("""
                {"eventId":"evt-%d","eventType":"%s","aggregateVersion":%d,
                 "payload":{"locationId":"%s","name":"Loc %s","code":"L-%d","status":"ACTIVE","active":true,
                            "parents":%s}}
                """.formatted(
    ...
    @Test
    @DisplayName("descendantsOf follows only the dimension's edge types downward, inclusive of the start node")
    void descendantsRespectDimension() {
        locationFact(REGION, 1);
        locationFact(FIN_ROOT, 1);
        locationFact(DISTRICT, 1, edge(REGION, "REGION"));
        locationFact(SHOP, 1, edge(DISTRICT, "PHYSICAL"), edge(FIN_ROOT, "FINANCIAL"));
        entityManager.flush();
        entityManager.clear();
        ...
        assertThat(service.descendantsOf(FIN_ROOT, Dimension.FINANCIAL)).containsExactlyInAnyOrder(FIN_ROOT, SHOP);
        assertThat(service.descendantsOf(id("99"), Dimension.OTHER)).isEmpty();
    }

    @Test
    @DisplayName("a deleted location drops its row and edges; its former children keep the edge but lose nothing else")
    void deleteRemovesRowAndEdges() {
        locationFact(REGION, 1);
        locationFact(SHOP, 1, edge(REGION, "PHYSICAL"));

        listener.onLocationEvent("""
                {"eventId":"evt-del","eventType":"location.location.deleted","payload":{"locationId":"%s"}}
                """.formatted(REGION));
        entityManager.flush();
        entityManager.clear();

        assertThat(extLocationReplicaRepository.findById(REGION)).isEmpty();
        assertThat(service.ancestorsOf(REGION)).isEqualTo(AncestorSets.EMPTY);
        // SHOP's own edge naming REGION is SHOP's fact to replace; it is left in place.
        assertThat(extLocationParentReplicaRepository.findByChildId(SHOP)).hasSize(1);
    }
