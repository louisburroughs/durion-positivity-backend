...
class ContractBehaviorIT extends BaseContractIntegrationTest {
        ...
        void setUp() {
                when(crmCustomerClient.getCustomerById(any()))
                                .thenReturn(Map.of("id", "customer-snapshot"));
                when(crmVehicleClient.getVehicleById(any()))
                                .thenReturn(Map.of("id", "vehicle-snapshot"));
        }
        ...
        void testCreateAppointment_HappyPath() throws Exception {
                String payload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-01-27T19:30:00Z", "2026-01-27T20:30:00Z", "BAY-1");
        ...
        void testGetAppointment_HappyPath() throws Exception {
                String payload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-02-15T15:00:00Z", "2026-02-15T16:30:00Z", "BAY-2");
                ...
                String appointmentId = objectMapper.readTree(createResult.getResponse().getContentAsString())
                                .get("appointmentId").asString();
        ...
        void testCreateAppointment_InvalidSourceType() throws Exception {
                String invalidPayload = createAppointmentPayloadWithSourceType(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-03-01T14:00:00Z", "2026-03-01T15:00:00Z", "BAY-1",
                                "INVALID_TYPE", "SRC-001");
                mockMvc.perform(withGatewayAuth(post("/v1/appointments")
                                .contentType(MediaType.APPLICATION_JSON)
        ...
        void testCreateAppointment_InvalidTimeRange() throws Exception {
                String invalidPayload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-03-15T20:00:00Z", "2026-03-15T19:00:00Z", "BAY-1");
                mockMvc.perform(withGatewayAuth(post("/v1/appointments")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(invalidPayload)
        ...
        void testCreateAppointment_Idempotent() throws Exception {
                ...
                String payload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-04-20T16:00:00Z", "2026-04-20T17:30:00Z", "BAY-3");
        ...
        void testCreateAppointment_HardConflict() throws Exception {
                String payload1 = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-05-01T14:00:00Z", "2026-05-01T15:00:00Z", "BAY-CONFLICT");
                mockMvc.perform(withGatewayAuth(post("/v1/appointments")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(payload1)
                ...
                String payload2 = createAppointmentPayload(CUSTOMER_2, VEHICLE_2, LOCATION,
                                "2026-05-01T14:30:00Z", "2026-05-01T15:30:00Z", "BAY-CONFLICT");
                mockMvc.perform(withGatewayAuth(post("/v1/appointments")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(payload2)
                                .header("X-Correlation-Id", "00000000-0000-0000-0000-000000000007")))
        ...
        void testCancelAppointment_StatusTransition() throws Exception {
                String payload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-05-15T19:00:00Z", "2026-05-15T20:00:00Z", "BAY-CANCEL");
                MvcResult createResult = mockMvc.perform(withGatewayAuth(post("/v1/appointments")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(payload)
        ...
        void testAppointment_TimestampFieldsPresent() throws Exception {
                String payload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-06-01T15:30:00Z", "2026-06-01T16:30:00Z", "BAY-1");
                mockMvc.perform(withGatewayAuth(post("/v1/appointments")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(payload)
        ...
        void testAppointment_ResponseFields() throws Exception {
                String payload = createAppointmentPayload(CUSTOMER_1, VEHICLE_1, LOCATION,
                                "2026-06-15T21:00:00Z", "2026-06-15T22:00:00Z", "BAY-1");
        ...
        private String createAppointmentPayload(String crmCustomerId, String crmVehicleId, String locationId,
                        String startAt, String endAt, String resourceId) {
        ...
        private String createAppointmentPayloadWithSourceType(String crmCustomerId, String crmVehicleId,
                        String locationId, String startAt, String endAt, String resourceId,
