...
@ActiveProfiles("test")
// Issue #74: H2 in-memory datasource + disable external startup dependencies for isolated integration tests
...
class ScheduleViewContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    /** Date used as the schedule view query target; all test appointments fall within this UTC day. */
    ...
    // Mocked to prevent context-startup failures; not invoked by schedule view operations
    @MockitoBean
    ...
    /**
     * Persists a SCHEDULED appointment with the given resource details and time window.
     *
    ...
    /**
     * SV1: Happy path — valid locationId + date returns 200 OK with a resources array.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
    ...
    void should_return_200_with_resources_array_for_valid_locationId_and_date() throws Exception {
        // Issue #74: set up a known appointment so the response has at least one resource entry
    ...
    /**
     * SV3: Filter by resourceType — returns 200 with only resources matching the requested type.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
    ...
    /**
     * SV7: Non-existent locationId → 404 Not Found with error code LOCATION_NOT_FOUND.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
    ...
    /**
     * SV8: Two appointments on the same resourceId and locationId with overlapping times
     * → hasConflict=true, severity=BLOCKING, conflictingEventIds populated and mutual.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
    ...
    void should_return_hasConflict_true_with_BLOCKING_severity_when_appointments_overlap() throws Exception {
        // Issue #74: two appointments on BAY-01 at TEST_LOCATION_ID with overlapping windows
    ...
    /**
     * SV9: Two appointments on the same resourceId but non-overlapping times → hasConflict=false.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
     ...
     * availabilityOverlayStatus=UNAVAILABLE and warnings containing HR_SYSTEM_UNAVAILABLE.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
    ...
    /**
     * SV11: Filter by resourceId with a value that exists in no appointment → 404 Not Found.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
    ...
    /**
     * SV12: Filter by resourceId — valid resourceId returns 200 with only events for that resource.
     * Currently fails with 501 because getScheduleView throws UnsupportedOperationException.
     */
    @Test
