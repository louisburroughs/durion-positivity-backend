...
public class MechanicAvailabilityServiceImpl implements MechanicAvailabilityService {
        ...
        public @NonNull MechanicAvailabilityResult queryAvailability(
                ...
                if (!windowStart.isBefore(windowEnd)) {
                        throw new IllegalArgumentException(
                                        "windowStart must be before windowEnd, got: " + windowStart + " >= "
                                                        + windowEnd);
                }

                var mechanic = mechanicRepository.findByPersonId(personId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Mechanic not found for personId: " + personId));
                ...
                List<HrScheduleBlock> hrBlocks = hrAvailabilityClient.getScheduleBlocks(personId, windowStart,
                                windowEnd);
                ...
                // AC5: Travel blocks
                List<TravelBlock> travelBlocks = travelBlockRepository.findOverlapping(personId, windowStart,
                                windowEnd);
                travelBlocks.stream()
         ...
         */
        private AvailabilityStatus computeStatus(List<ConflictBlock> conflicts, Instant windowStart,
                        Instant windowEnd) {
        ...
        private List<ConflictBlock> buildOffShiftConflicts(
                ...
                List<TimeRange> shiftRanges = hrBlocks.stream()
                                ...
                                .map(b -> new TimeRange(maxInstant(b.getStartTime(), windowStart),
                                                minInstant(b.getEndTime(), windowEnd)))
