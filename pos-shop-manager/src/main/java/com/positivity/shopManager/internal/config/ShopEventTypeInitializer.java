...
/**
 * Initializes all event types for the pos-shop-manager module on application
 * startup.
 * Uses the pos-events upsert endpoint for idempotent registration.
 */
@Component
public class ShopEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(ShopEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;

    public ShopEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-shop-manager");
    }

    @Override
    public void run(ApplicationArguments args) {
        log.info("Registering {} shop manager event types", ShopEventTypes.all().size());

        initializerSupport.registerEventTypes(
                ShopEventTypes.all(),
                this::registerEventType);

        log.info("Shop manager event type registration complete");
    }
