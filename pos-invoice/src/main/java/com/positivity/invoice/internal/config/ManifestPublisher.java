 ...
 *
 * <p>Each closed window gets one {@link ReconciliationManifestV1} on {@code invoice.manifest.v1}
 ...
 * request an outbox replay over {@code invoice.commands.v1} on drift.
 *
...
public class ManifestPublisher {
    ...
    @Value("${pos.invoice.kafka.events-topic:invoice.events.v1}")
    private String eventsTopic;

    @Value("${pos.invoice.manifest.topic:invoice.manifest.v1}")
    private String manifestTopic;

    /** Reconciliation window length. */
    @Value("${pos.invoice.manifest.window:PT1H}")
    private Duration window;

    /** How long after a window closes before its manifest is published. */
    @Value("${pos.invoice.manifest.grace:PT5M}")
    private Duration grace;

    @Value("${pos.invoice.manifest.send-timeout-ms:10000}")
    ...
    public ManifestPublisher(
        ...
        this.publishedCounter = registry == null
                ? null
                : Counter.builder("invoice.manifest.published")
                        .description("Reconciliation manifests published")
                        .register(registry);
        this.failedCounter = registry == null
                ? null
                : Counter.builder("invoice.manifest.publish.failures")
