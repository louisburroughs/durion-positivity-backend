...
/**
 * Consumer-side reconciliation for the {@code ext_location} replica (ADR-0044 §4, issue #892).
 *
 * <p>For every {@link ReconciliationManifestV1} on {@code location.manifest.v1}, recomputes the
 * same count + checksum from {@code processed_events} (owner {@code location}) — window
 ...
 * {@code location.outbox.replay-requested} command for the window; the replayed events are
 ...
 *
 * <p>The owner's manifest counts every fact in the window (including storage-location facts this
 * module ignores), which is why {@link LocationEventsListener} records every eventId it sees.
 */
@Slf4j
@Component
@ConditionalOnProperty(prefix = "pos.invoice.kafka", name = "enabled", havingValue = "true")
public class LocationManifestListener {
    ...
    @Value("${pos.invoice.kafka.location-commands-topic:location.commands.v1}")
    private String locationCommandsTopic;

    public LocationManifestListener(
    ...
    @KafkaListener(
            topics = "${pos.invoice.kafka.location-manifest-topic:location.manifest.v1}",
            groupId = "${pos.invoice.kafka.location-manifest-consumer-group:pos-invoice-location-manifests}")
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=location window=[{}, {}): manifests"
