...
public class ReceiptController {
    ...
    @GetMapping("/{invoiceId}/receipts/{receiptId}")
    @PreAuthorize("hasAuthority('" + InvoicePermissions.VIEW + "')")
    @EmitEvent(id = "INVOICE_RECEIPT_VIEW", apiVersion = "1")
    @Operation(operationId = "getReceipt", summary = "Get Receipt Detail", description = """
                    Returns the full detail of a single receipt: its reference and status, the invoice and \
                    payment intent it documents, the paid amount, the payment gateway and gateway reference \
                    captured on the payment intent, the cashier, terminal and template that produced it, its \
                    delivery status, and its reprint history.
                    Use this tool to render the frontend receipt page for a receipt already generated by \
                    generateReceipt; do not use it to check whether a receipt exists for an invoice — list the \
                    invoice's receipts for that.
    ...
    @ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = ApiError.class)))
    public ResponseEntity<ReceiptViewResponse> getReceipt(
            @PathVariable @NonNull UUID invoiceId, @PathVariable @NonNull UUID receiptId) {
        return ResponseEntity.ok(receiptService.getReceipt(invoiceId, receiptId));
    }
