...
CREATE TABLE billing_rules ( purchase_order_required boolean NOT NULL, version integer NOT NULL, created_at timestamp(6) with time zone NOT NULL, updated_at timestamp(6) with time zone NOT NULL, id uuid NOT NULL, invoice_delivery_method character varying(20) NOT NULL, invoice_grouping_strategy character varying(30) NOT NULL, party_id character varying(36) NOT NULL, updated_by character varying(36) NOT NULL, payment_terms_code character varying(50) NOT NULL, CONSTRAINT billing_rules_payment_terms_code_check CHECK (((payment_terms_code)::text = ANY ((ARRAY['DUE_ON_RECEIPT'::character varying, 'NET_10'::character varying, 'NET_15'::character varying, 'NET_30'::character varying, 'NET_45'::character varying, 'NET_60'::character varying])::text[]))), CONSTRAINT billing_rules_invoice_delivery_method_check CHECK (((invoice_delivery_method)::text = ANY ((ARRAY['EMAIL'::character varying, 'PORTAL'::character varying, 'MAIL'::character varying])::text[]))), CONSTRAINT billing_rules_invoice_grouping_strategy_check CHECK (((invoice_grouping_strategy)::text = ANY ((ARRAY['PER_WORKORDER'::character varying, 'PER_VEHICLE'::character varying, 'SINGLE_INVOICE'::character varying])::text[]))) );
...
CREATE TABLE invoices ( adjustments_amount numeric(19,4) NOT NULL, subtotal numeric(19,4) NOT NULL, tax_amount numeric(19,4) NOT NULL, total_amount numeric(19,4) NOT NULL, version integer NOT NULL, created_at timestamp(6) with time zone NOT NULL, finalized_at timestamp(6) with time zone, updated_at timestamp(6) with time zone NOT NULL, approval_id uuid, estimate_id uuid, id uuid NOT NULL, workorder_id uuid NOT NULL, status character varying(20) NOT NULL, customer_id character varying(64), finalized_by character varying(64), invoice_number character varying(64), due_date date, payment_terms_code character varying(20), CONSTRAINT invoices_finalized_due_date_check CHECK (((status)::text NOT IN ('FINALIZED', 'POSTED')) OR (due_date IS NOT NULL AND payment_terms_code IS NOT NULL)), CONSTRAINT invoices_status_check CHECK (((status)::text = ANY ((ARRAY['DRAFT'::character varying, 'FINALIZED'::character varying, 'POSTED'::character varying, 'ERROR'::character varying])::text[]))) );
