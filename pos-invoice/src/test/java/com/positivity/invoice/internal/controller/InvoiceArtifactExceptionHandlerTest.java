...
/**
 * Unit tests for {@link InvoiceArtifactExceptionHandler} (ADR-0017 §4, ADR-0056 §1, #1729).
 */
class InvoiceArtifactExceptionHandlerTest {

    private static final String CORRELATION_HEADER = "X-Correlation-Id";
    private static final Instant NOW = Instant.parse("2026-08-25T09:00:00Z");

    /**
     * Proves every {@code @ExceptionHandler} on {@link InvoiceArtifactExceptionHandler} carries
     ...
     * when this guard was added — every handler routes through the private {@code error} helper,
     * which sets the {@code X-Correlation-Id} header — so no production code changed; this only
     * pins the behavior against regression and guards a future handler forgetting the header.
     */
    @Nested
    ...
    class XCorrelationIdHeader {
        ...
        /**
         * One entry per {@code @ExceptionHandler} method on {@link InvoiceArtifactExceptionHandler}
         * (not per mapped exception type — {@code handleNotFound} maps two types via a single
         * method). Uses a standalone handler instance so this factory method can stay static, as
         * required by {@code @MethodSource} outside a {@code PER_CLASS} test instance lifecycle.
         */
        private static Stream<Named<HandlerInvocation>> handlerInvocations() {
            InvoiceArtifactExceptionHandler handler =
                    new InvoiceArtifactExceptionHandler(Clock.fixed(NOW, ZoneOffset.UTC));

            return Stream.of(
                    Named.of("handleNotFound", (HandlerInvocation) request -> handler.handleNotFound(
                            new ArtifactNotFoundException("Artifact does not belong to invoice X"), request)),
                    Named.of("handleInvalidToken", (HandlerInvocation) request -> handler.handleInvalidToken(
                            new InvalidTokenException("Invalid download token signature"), request)),
                    Named.of("handleBadRequest", (HandlerInvocation) request -> handler.handleBadRequest(
                            new InvoiceRequestValidationException("Malformed artifact reference"), request)));
        }
        ...
        @Test
        @DisplayName(
                "every @ExceptionHandler method on InvoiceArtifactExceptionHandler has a matching MethodSource entry")
        void everyHandlerMethodIsCovered() {
            long handlerMethodCount = Arrays.stream(InvoiceArtifactExceptionHandler.class.getDeclaredMethods())
            ...
            assertThat(methodSourceEntryCount)
                    .as("A new @ExceptionHandler method was added to InvoiceArtifactExceptionHandler without a "
                            + "matching entry in XCorrelationIdHeader#handlerInvocations() in "
                            + "InvoiceArtifactExceptionHandlerTest — add one so the X-Correlation-Id header "
