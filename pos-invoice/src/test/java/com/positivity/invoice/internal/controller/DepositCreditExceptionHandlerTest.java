...
/**
 * Unit tests for {@link DepositCreditExceptionHandler} (ADR-0017 §4, ADR-0056 §1, #1729).
 */
class DepositCreditExceptionHandlerTest {

    private static final String CORRELATION_HEADER = "X-Correlation-Id";
    private static final Instant NOW = Instant.parse("2026-08-25T09:00:00Z");

    /**
     * Proves every {@code @ExceptionHandler} on {@link DepositCreditExceptionHandler} carries the
    ...
    class XCorrelationIdHeader {
        ...
        /**
         * One entry per {@code @ExceptionHandler} method on {@link DepositCreditExceptionHandler}.
        ...
        private static Stream<Named<HandlerInvocation>> handlerInvocations() {
            DepositCreditExceptionHandler handler = new DepositCreditExceptionHandler(Clock.fixed(NOW, ZoneOffset.UTC));

            return Stream.of(
                    Named.of("handleNotFound", (HandlerInvocation) request ->
                            handler.handleNotFound(new DepositCreditNotFoundException(UUID.randomUUID()), request)),
                    Named.of("handleValidation", (HandlerInvocation) request -> handler.handleValidation(
                            new InvoiceRequestValidationException("Unknown deposit source type: X"), request)));
        }
        ...
        @Test
        @DisplayName(
                "every @ExceptionHandler method on DepositCreditExceptionHandler has a matching MethodSource entry")
        void everyHandlerMethodIsCovered() {
            long handlerMethodCount = Arrays.stream(DepositCreditExceptionHandler.class.getDeclaredMethods())
            ...
            assertThat(methodSourceEntryCount)
                    .as("A new @ExceptionHandler method was added to DepositCreditExceptionHandler without a "
                            + "matching entry in XCorrelationIdHeader#handlerInvocations() in "
                            + "DepositCreditExceptionHandlerTest — add one so the X-Correlation-Id header contract "
