...
/**
 * Unit tests for {@link InvoiceSearchExceptionHandler} (ADR-0017 §4, ADR-0056 §1, #1729).
 */
class InvoiceSearchExceptionHandlerTest {

    private static final String CORRELATION_HEADER = "X-Correlation-Id";
    private static final Instant NOW = Instant.parse("2026-08-25T09:00:00Z");

    /**
     * Proves every {@code @ExceptionHandler} on {@link InvoiceSearchExceptionHandler} carries the
     ...
     * guard was added — its single handler builds its own {@code ResponseEntity} with the {@code
    ...
    class XCorrelationIdHeader {
        ...
        /**
         * One entry per {@code @ExceptionHandler} method on {@link InvoiceSearchExceptionHandler}.
        ...
        private static Stream<Named<HandlerInvocation>> handlerInvocations() {
            InvoiceSearchExceptionHandler handler = new InvoiceSearchExceptionHandler(Clock.fixed(NOW, ZoneOffset.UTC));

            return Stream.of(
                    Named.of("handleIllegalArgument", (HandlerInvocation) request -> handler.handleIllegalArgument(
                            new InvoiceRequestValidationException("issuedTo must not be before issuedFrom"), request)));
        }
        ...
        @Test
        @DisplayName(
                "every @ExceptionHandler method on InvoiceSearchExceptionHandler has a matching MethodSource entry")
        void everyHandlerMethodIsCovered() {
            long handlerMethodCount = Arrays.stream(InvoiceSearchExceptionHandler.class.getDeclaredMethods())
            ...
            assertThat(methodSourceEntryCount)
                    .as("A new @ExceptionHandler method was added to InvoiceSearchExceptionHandler without a "
                            + "matching entry in XCorrelationIdHeader#handlerInvocations() in "
                            + "InvoiceSearchExceptionHandlerTest — add one so the X-Correlation-Id header contract "
