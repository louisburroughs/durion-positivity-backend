...
/**
 * Unit tests for {@link InvoiceAnalyticsExceptionHandler} (ADR-0017 §4, ADR-0056 §1, #1729).
 */
class InvoiceAnalyticsExceptionHandlerTest {

    private static final String CORRELATION_HEADER = "X-Correlation-Id";
    private static final Instant NOW = Instant.parse("2026-08-25T09:00:00Z");

    /**
     * Proves every {@code @ExceptionHandler} on {@link InvoiceAnalyticsExceptionHandler} carries
     ...
     * when this guard was added — its single handler builds its own {@code ResponseEntity} with
     * the {@code X-Correlation-Id} header — so no production code changed; this only pins the
    ...
    class XCorrelationIdHeader {
        ...
        /**
         * One entry per {@code @ExceptionHandler} method on {@link InvoiceAnalyticsExceptionHandler}.
        ...
        private static Stream<Named<HandlerInvocation>> handlerInvocations() {
            InvoiceAnalyticsExceptionHandler handler =
                    new InvoiceAnalyticsExceptionHandler(Clock.fixed(NOW, ZoneOffset.UTC));

            return Stream.of(
                    Named.of("handleIllegalArgument", (HandlerInvocation) request -> handler.handleIllegalArgument(
                            new InvoiceRequestValidationException("endDate must not be before startDate"), request)));
        }
        ...
        @Test
        @DisplayName(
                "every @ExceptionHandler method on InvoiceAnalyticsExceptionHandler has a matching MethodSource entry")
        void everyHandlerMethodIsCovered() {
            long handlerMethodCount = Arrays.stream(InvoiceAnalyticsExceptionHandler.class.getDeclaredMethods())
            ...
            assertThat(methodSourceEntryCount)
                    .as("A new @ExceptionHandler method was added to InvoiceAnalyticsExceptionHandler without a "
                            + "matching entry in XCorrelationIdHeader#handlerInvocations() in "
                            + "InvoiceAnalyticsExceptionHandlerTest — add one so the X-Correlation-Id header "
