...
public class PartyRelationshipService {
    ...
    public GetCommercialAccountContactsResponse getContactsForCommercialAccount(
        ...
        List<GetCommercialAccountContactsResponse.ContactWithRole> contacts = relationships.stream()
                .map(rel -> {
                    ...
                    String email = getPrimaryContactValue(person.getPersonId(), ContactPointType.EMAIL);
                    String phone = getPrimaryContactValue(person.getPersonId(), ContactPointType.PHONE_MOBILE);
                    if (phone == null) {
                        phone = getPrimaryContactValue(person.getPersonId(), ContactPointType.PHONE_WORK);
                    }
    ...
    public void deactivateRelationship(@NonNull UUID relationshipId, UUID userId) {
        ...
        log.info("Deactivated relationship {} for party {} and person {}",
                relationshipId, relationship.getFromParty().getId(), relationship.getToPerson().getPersonId());
    }
    ...
    private String getPrimaryContactValue(UUID personId, ContactPointType type) {
        ContactPoint primary = contactPointRepository.findByPersonPersonIdAndContactTypeAndIsPrimaryTrue(personId,
                type);
        if (primary != null) {
