...
public class VehicleManifestListener {
    ...
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=vehicle window=[{}, {}): manifests"
    ...
    private void requestReplay(@NonNull ReconciliationManifestV1 manifest, @NonNull UUID tenantId) {
        try {
            ...
            kafkaTemplate.send(TenantKafkaHeaders.record(
                    vehicleCommandsTopic, manifest.windowStartUtc().toString(), command, tenantId));
        } catch (Exception e) {
