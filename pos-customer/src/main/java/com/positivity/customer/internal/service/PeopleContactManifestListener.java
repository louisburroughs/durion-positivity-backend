...
/**
 * Consumer-side reconciliation for the person identity replica (ADR-0044 §4, issue #877).
 *
 * <p>For every {@link ReconciliationManifestV1} on {@code people-contact.manifest.v1}, recomputes the
 * same count + checksum from {@code processed_events} (owner {@code people-contact}) — window membership
 ...
 * (Prometheus: {@code replica_drift_total{owner="people-contact"}}) and publishes a
 * {@code people-contact.outbox.replay-requested} command for the window; the replayed events are
...
public class PeopleContactManifestListener {
    ...
    @Value("${pos.customer.kafka.people-contact-commands-topic:people-contact.commands.v1}")
    ...
    @KafkaListener(
            topics = "${pos.customer.kafka.people-contact-manifest-topic:people-contact.manifest.v1}",
            groupId =
                    "${pos.customer.kafka.people-contact-manifest-consumer-group:pos-customer-people-contact-manifests}")
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=people-contact window=[{}, {}): manifests"
    ...
    private void requestReplay(@NonNull ReconciliationManifestV1 manifest, @NonNull UUID tenantId) {
        try {
            ...
            kafkaTemplate.send(TenantKafkaHeaders.record(
                    peopleContactCommandsTopic, manifest.windowStartUtc().toString(), command, tenantId));
        } catch (Exception e) {
