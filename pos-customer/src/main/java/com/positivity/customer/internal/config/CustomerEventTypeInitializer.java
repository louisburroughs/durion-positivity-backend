...
/**
 * Initializes all event types for the pos-customer module on application
 * startup.
 */
@Component
public class CustomerEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(CustomerEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;
    private final String apiSecret;

    public CustomerEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-customer");
        this.apiSecret = apiSecret;
    }

    @Override
    public void run(ApplicationArguments args) {
        log.info("Registering {} customer event types", CustomerEventTypes.all().size());

        initializerSupport.registerEventTypes(
                CustomerEventTypes.all(),
                this::registerEventType);

        log.info("Customer event type registration complete");
    }
