 ...
 *
 * <p>It exists as its own type rather than as fields on a base class because several base classes
 * and standalone ITs need it — the {@code @SpringBootTest}
 * {@link com.positivity.customer.tenancy.PostgresTenancyTestBase}, the {@code @DataJpaTest}
 * {@link PostgresSliceTestBase}, the service-level {@link PostgresIntegrationTestBase}, and the
 * migration IT — and one container bootstrap per test class is a copy too many.
 *
 * <p>Requires Docker.
 */
public final class CustomerPostgresContainer {
     ...
     * every Spring context it has loaded, each with its own pool: the module has more Postgres-backed
     * context shapes than 100 connections divided by a default pool, and the overflow surfaces as
     * {@code FATAL: remaining connection slots are reserved} while a context that has nothing to do
     * with the failing test is being built.
     */
    ...
    private CustomerPostgresContainer() {}
    ...
    /**
     * Points a Spring context at the container as the schema owner, for the handful of tests that
     * read the catalog or assert on migration output rather than on application behaviour.
     *
     * @param registry the registry the calling {@code @DynamicPropertySource} was handed
     */
    public static void registerOwnerDataSourceProperties(DynamicPropertyRegistry registry) {
        start();
        registry.add("spring.datasource.url", POSTGRES::getJdbcUrl);
