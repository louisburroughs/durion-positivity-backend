...
/**
 * Unit tests for {@link VehicleManifestListener} (ADR-0044 §4, #843) — the consumer-side
 * reconciliation of the {@code ext_vehicle} replica against the owner's manifests, recomputed
 * from {@code processed_events} rows of the {@code vehicle} owner.
 */
class VehicleManifestListenerTest {

    private static final Instant WINDOW_START = Instant.parse("2026-07-12T10:00:00Z");
    private static final Instant WINDOW_END = Instant.parse("2026-07-12T11:00:00Z");
    ...
    private VehicleManifestListener listener;
    ...
    private String manifestMessage(long eventCount, String checksum) {
        return """
                {
                  "eventId": "%s",
                  "eventType": "vehicle.reconciliation.manifest",
                  "schemaVersion": 1,
                  ...
                  "payload": {
                    ...
                    "eventCount": %d,
                    "eventIdsChecksum": "%s",
                    "eventTypeCounts": {"vehicle.vehicle.updated": %d}
                  }
                }
                """.formatted(
