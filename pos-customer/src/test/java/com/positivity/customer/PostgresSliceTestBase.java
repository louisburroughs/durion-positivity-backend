 ...
 * {@link CustomerPostgresContainer}, with Hibernate {@code validate} and the pool connected as the
 * non-owner {@code pos_app} role — the arrangement alpha runs (ADR-0062 §3).
 *
 * <p>It replaces the H2 database these slices used to boot, which ran Hibernate's own
 * {@code create-drop} DDL with Flyway switched off. Nothing tied that schema to the flattened
 * Postgres baseline the application actually meets, so a slice could assert against DDL that does
 * not ship — and could not see the dialect differences that matter, which is how an all-optional
 * JPQL filter that fails on PostgreSQL for every call stayed green here (issue #1891).
 *
