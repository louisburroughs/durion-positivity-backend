package com.durion.inventory.putaway;

import com.durion.inventory.exception.LocationAtCapacityException;
import com.durion.inventory.exception.LocationNotValidForSkuException;
import com.durion.inventory.exception.NoOnHandAtSourceLocationException;
 ...
 * 
 * <p>This implementation enforces the business rules defined in clarification #229.
 * 
 * <p><strong>Note:</strong> This is a stub implementation that demonstrates the validation
...
public class PutawayValidationServiceImpl implements PutawayValidationService {
    ...
    public ValidationResult validateLocationCompatibility(String destinationLocationId, String skuId) {
        ...
        //     throw new LocationNotValidForSkuException(destinationLocationId, skuId, reason);
        // }
    ...
    public ValidationResult validateSourceOnHand(String sourceLocationId, String skuId, int quantity) {
        ...
        // Example validation logic (to be replaced with actual implementation):
        // int onHandQuantity = inventoryRepository.getOnHandQuantity(sourceLocationId, skuId);
        //
        // if (onHandQuantity == 0) {
    ...
    public ValidationResult validatePutawayExecution(PutawayExecutionRequest request) {
        ...
        try {
            ...
            if (!sourceValidation.isValid()) {
                sourceValidation.getErrors().forEach(err -> 
                    result.addError(err.getErrorCode(), err.getMessage()));
            }
        ...
        if (!request.isOverrideLocationCompatibility()) {
            try {
                ...
                if (!compatValidation.isValid()) {
                    compatValidation.getErrors().forEach(err -> 
                        result.addError(err.getErrorCode(), err.getMessage()));
                }
        ...
        if (!request.isOverrideCapacity()) {
            try {
                ...
                capacityValidation.getWarnings().forEach(warn -> 
                    result.addWarning(warn.getCode(), warn.getMessage()));
                    
                if (!capacityValidation.isValid()) {
                    capacityValidation.getErrors().forEach(err -> 
                        result.addError(err.getErrorCode(), err.getMessage()));
                }
