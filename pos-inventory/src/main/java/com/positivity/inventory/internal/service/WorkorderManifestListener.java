...
/**
 * Consumer-side reconciliation for the inventory workorder replicas (ADR-0044 §4, issue #897).
 *
...
public class WorkorderManifestListener {
    ...
    @Value("${pos.inventory.kafka.workorder-commands-topic:workorder.commands.v1}")
    ...
    @KafkaListener(
            topics = "${pos.inventory.kafka.workorder-manifest-topic:workorder.manifest.v1}",
            groupId = "${pos.inventory.kafka.workorder-manifest-consumer-group:pos-inventory-workorder-manifests}")
