...
public class AsnServiceImpl implements AsnService {
        ...
        public @NonNull AsnResponse createAsn(@NonNull CreateAsnRequest request, @NonNull String actorId) {
                ...
                asnRepository.findByVendorIdAndAsnReferenceNumber(request.getVendorId(),
                                request.getAsnReferenceNumber())
                                .ifPresent(existing -> {
                                        throw new DuplicateAsnException(
                                                        "DUPLICATE_ASN: ASN with this reference already exists");
                                });
        ...
        public @NonNull GoodsReceiptResponse createGoodsReceipt(
                ...
                if (request.getAsnId() != null) {
                        asn = asnRepository.findById(request.getAsnId())
                                        .orElseThrow(() -> new ResourceNotFoundException("Asn",
                                                        request.getAsnId().toString()));
                }
                ...
                for (CreateGoodsReceiptLineRequest line : request.getLines()) {
                        InventoryLedgerEntry entry = InventoryLedgerEntry.builder()
                                        ...
                                        .quantityAfter(
                                                        calculateQuantityAfter(line.getSku(), request.getLocationId(),
                ...
                purchaseOrder.setStatus(
                                nextOpenBalance == 0L ? PurchaseOrderStatus.FULLY_RECEIVED
        ...
        private int calculateQuantityAfter(@NonNull String stockItemId, @NonNull UUID locationId,
                ...
                Integer onHand = inventoryLedgerEntryRepository.calculateOnHandQuantityAtLocation(stockItemId,
                                locationId);
