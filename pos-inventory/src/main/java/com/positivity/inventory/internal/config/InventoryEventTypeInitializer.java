...
/**
 * Initializes all event types for the pos-inventory module on application
 * startup.
 * Uses the pos-events upsert endpoint for idempotent registration.
 */
@Component
public class InventoryEventTypeInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(InventoryEventTypeInitializer.class);

    private final RestClient restClient;
    private final EventTypeInitializerSupport initializerSupport;

    public InventoryEventTypeInitializer(
        ...
        this.restClient = restClientBuilder
                ...
                .build();
        this.initializerSupport = new EventTypeInitializerSupport("pos-inventory");
    }

    @Override
    public void run(ApplicationArguments args) {
        log.info("Registering {} inventory event types", InventoryEventTypes.all().size());

        initializerSupport.registerEventTypes(
                InventoryEventTypes.all(),
