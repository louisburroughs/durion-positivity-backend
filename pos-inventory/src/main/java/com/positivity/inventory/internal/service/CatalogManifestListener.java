...
public class CatalogManifestListener {
    ...
    public void onManifest(@NonNull String message) {
        ...
        if (tenantId == null) {
            ...
            log.warn(
                    "Skipping reconciliation manifest without tenantId owner=catalog window=[{}, {}): manifests"
        ...
        countDrift(tenantId);
        log.warn(
                "Replica drift detected owner=catalog tenant={} window=[{}, {}) expectedCount={} observedCount={}"
    ...
    private void requestReplay(@NonNull ReconciliationManifestV1 manifest, @NonNull UUID tenantId) {
        try {
            ...
            kafkaTemplate.send(TenantKafkaHeaders.record(
                    catalogCommandsTopic, manifest.windowStartUtc().toString(), command, tenantId));
        } catch (Exception e) {
