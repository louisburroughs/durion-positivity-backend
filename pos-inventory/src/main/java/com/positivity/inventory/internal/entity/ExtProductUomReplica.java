...
/**
 * Read-only per-product UoM conversion row carried on {@code catalog.product.updated} v2 facts
 * (odoo-parity B1, #1033). Every fact carries the product's full conversion set — the consumer
 * replaces the product's rows wholesale ({@code ext_workorder_part} pattern). The owner allows
 * exactly one row per (product, uomCode), so that pair is the natural key.
 *
