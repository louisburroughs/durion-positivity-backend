 ...
 * {@code POS_INVENTORY_KAFKA_ENABLED} environment variable: {@link ApplicationContextRunner}'s
 ...
 * name-mapping.
 */
@DisplayName("KafkaEnabledGuard — fail loud when pos.inventory.kafka.enabled is off (#2192)")
class KafkaEnabledGuardTest {
    ...
    @Test
    @DisplayName("prod + pos.inventory.kafka.enabled resolved false fails startup naming the property")
    void prodWithFlagResolvedFalseFailsStartup() {
        runner("spring.profiles.active=prod", "pos.inventory.kafka.enabled=false")
                .run(context -> {
    ...
    @Test
    @DisplayName("alpha + pos.inventory.kafka.enabled resolved false fails startup naming the property")
    void alphaWithFlagResolvedFalseFailsStartup() {
        runner("spring.profiles.active=alpha", "pos.inventory.kafka.enabled=false")
                .run(context -> {
                    assertThat(context).hasFailed();
                    assertFailureNamesProperty(context);
                });
    ...
    @Test
    @DisplayName("dev + pos.inventory.kafka.enabled=false loads: dev is exempt by design")
    void devWithFlagFalseLoads() {
        runner("spring.profiles.active=dev", "pos.inventory.kafka.enabled=false")
                .run(context -> {
