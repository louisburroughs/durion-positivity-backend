...
class CrossDockContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    private MockHttpServletRequestBuilder withReceivingOnlyAuth(MockHttpServletRequestBuilder requestBuilder) {
        return requestBuilder
                ...
                .header("X-Authorities", "inventory:receiving:create"); // intentionally missing
        // issue:parts
    }
    ...
    @Test
    @DisplayName(
            "AC1: POST cross-dock with full expected quantity returns 200 with crossDockedQuantity=10 and workorderId")
    void crossDock_fullExactQuantity_returns200WithWorkorderAndQuantity() throws Exception {
        ...
        mockMvc.perform(withReceivingAuth(post(
                                "/v1/inventory/receiving/sessions/{sessionId}/lines/{lineId}/cross-dock",
                                sessionId,
                                lineId)
    ...
    void crossDock_missingIssuePartsAuthority_returns403Forbidden() throws Exception {
        ...
        mockMvc.perform(withReceivingOnlyAuth(post(
                                "/v1/inventory/receiving/sessions/{sessionId}/lines/{lineId}/cross-dock",
                                sessionId,
                                lineId)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void crossDock_partMismatchWithoutOverridePermission_returns403() throws Exception {
        ...
        when(receivingService.crossDockLineToWorkorder(eq(sessionId), eq(lineId), any(), any()))
                .thenThrow(
                        new PartMatchPermissionException("PART_MISMATCH_WITH_WORKORDER: override permission required"));
