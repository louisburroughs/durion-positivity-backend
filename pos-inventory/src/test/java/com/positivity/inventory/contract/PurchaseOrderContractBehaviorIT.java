...
class PurchaseOrderContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void draftPoReceiptAttempt_returns409() throws Exception {
        ...
        mockMvc.perform(withPurchaseOrderReceiveAuth(post("/v1/inventory/purchase-orders/{poId}/receive", poId)
    ...
    void partialReceipt_decrementsOpenQuantityAndBalance() throws Exception {
        ...
        String receiveBody = objectMapper.writeValueAsString(java.util.Map.of(
                "lineReceipts", List.of(java.util.Map.of("lineId", lineId.toString(), "receivedQty", 4))));
    ...
    void fullReceipt_transitionsToFullyReceived() throws Exception {
        ...
        String receiveBody = objectMapper.writeValueAsString(java.util.Map.of(
                "lineReceipts", List.of(java.util.Map.of("lineId", lineId.toString(), "receivedQty", 10))));
    ...
    void createPo_taxTotalsAggregate_grandTotalEqualsSubtotalPlusTax() throws Exception {
        ...
        CreatePurchaseOrderRequest request = new CreatePurchaseOrderRequest(
                ...
                "USD",
                "NET30",
                LocalDate.now(TEST_CLOCK).plusDays(30),
                null,
                "procurement-agent",
                "Test PO",
                List.of(line));
        ...
        mockMvc.perform(withPurchaseOrderWriteAuth(post("/v1/inventory/purchase-orders")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listOpenPosForVendor_returnsOpenBalanceAndLineQuantities() throws Exception {
        ...
        mockMvc.perform(withPurchaseOrderViewAuth(get("/v1/inventory/purchase-orders")
    ...
    void poRevision_incrementsVersionNumber() throws Exception {
        ...
        RevisePurchaseOrderRequest reviseRequest = new RevisePurchaseOrderRequest(
                LocalDate.now(TEST_CLOCK),
                "NET45",
                LocalDate.now(TEST_CLOCK).plusDays(45),
                null,
                "procurement-agent",
                "Revised due to price update",
                List.of(revisedLine),
        ...
        when(purchaseOrderService.revisePurchaseOrder(
                        eq(poId), any(RevisePurchaseOrderRequest.class), any(String.class)))
                .thenReturn(revisedResponse);

        mockMvc.perform(withPurchaseOrderWriteAuth(post("/v1/inventory/purchase-orders/{poId}/revisions", poId)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(purchaseOrderService)
                .revisePurchaseOrder(eq(poId), any(RevisePurchaseOrderRequest.class), any(String.class));
    }
    ...
    @Test
    @DisplayName(
            "POST /purchase-orders/{poId}/approve → 200 + service.approvePurchaseOrder called with required payload")
    ...
    void createPo_missingAuthority_returns403() throws Exception {
        ...
        // insufficient).
        mockMvc.perform(withPurchaseOrderViewAuth(post("/v1/inventory/purchase-orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody)))
    ...
    private MockHttpServletRequestBuilder withPurchaseOrderWriteAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "po-test-user")
                .header(
                        "X-Authorities",
                        String.join(",", "inventory:purchase_order:create", "inventory:purchase_order:view"));
    }

    private MockHttpServletRequestBuilder withPurchaseOrderViewAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "po-test-user").header("X-Authorities", "inventory:purchase_order:view");
    }
    ...
    private MockHttpServletRequestBuilder withPurchaseOrderApproveAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "po-test-user")
                .header(
                        "X-Authorities",
                        String.join(",", "inventory:purchase_order:approve", "inventory:purchase_order:view"));
    }
    ...
    private MockHttpServletRequestBuilder withPurchaseOrderReceiveAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "po-test-user")
                .header(
                        "X-Authorities",
                        String.join(",", "inventory:purchase_order:receive", "inventory:purchase_order:view"));
    }
