 ...
 *
 * <p>Tests CC1, CC3, CC4 are intentionally RED: the {@code ConsumptionController}
 * is a hardcoded stub and does not delegate to {@code ConsumptionService}. CC1 fails
 * because the consumptionId in the response is randomly generated (not sourced from the
 * mocked service). CC3 and CC4 fail because the stub never invokes the service to raise
 * domain exceptions, returning 201 instead of 422/400.
 *
 * <p>CC2 (missing auth → 403) is expected to pass in RED phase because
 ...
 *   <li>ADR-0014: internal service security — all state-changing endpoints require gateway auth</li>
 *   <li>ADR-0017: HTTP response codes: 201 Created, 400 Bad Request, 403 Forbidden, 422 Unprocessable Entity</li>
...
class ConsumptionContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    // ─── CC1: POST /v1/inventory/consumption — 201 Created, consumptionId from service ──

    /**
     * CC1: Verifies that POST /v1/inventory/consumption with a valid request body and
     * gateway auth returns 201 Created, and that the {@code consumptionId} in the response
     * originates from the {@code ConsumptionService} mock (not a controller-generated UUID).
     *
     * <p>RED: {@code ConsumptionController} does not delegate to {@code ConsumptionService};
     * it generates its own random UUID, so the {@code consumptionId} assertion fails.
     *
     * Issue: #178
     */
    @Test
     ...
     *
     * <p>This test is expected to PASS in the RED phase because
    ...
    /**
     * CC3: Verifies that POST /v1/inventory/consumption returns 422 Unprocessable Entity
     * when the referenced pick task is not in PICKED status (service throws
     * {@link WorkorderConsumptionException}).
     *
     * <p>RED: {@code ConsumptionController} does not call {@code ConsumptionService};
    ...
    void CC3_consumePickedItems_unpickedTask_returns422() throws Exception {
        // Issue #178: CC3 — service throws WorkorderConsumptionException for non-PICKED tasks;
    ...
    /**
     * CC4: Verifies that POST /v1/inventory/consumption returns 400 Bad Request when
     * the request body has a null workorderId.
     *
     * <p>RED: {@code ConsumptionController} does not validate the request body;
