...
import com.positivity.inventory.internal.refdata.service.InventoryReferenceDataService;
...
class InventoryReferenceDataContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void listLocations_withAuth_returns200() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(get("/v1/inventory/locations"))).andExpect(status().isOk());
    }
    ...
    void listLocations_withoutAuth_returnsUnauthorizedOrForbidden() throws Exception {
        mockMvc.perform(get("/v1/inventory/locations")).andExpect(status().is4xxClientError());
    }
    ...
    void listStorageLocations_withAuth_returns200() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(get("/v1/inventory/storage-locations"))).andExpect(status().isOk());
    }
    ...
    void listLocationZones_withAuth_returns200() throws Exception {
        ...
        mockMvc.perform(withGatewayAuth(get("/v1/inventory/location-zones"))).andExpect(status().isOk());
    }
}
