...
class AsnContractBehaviorIT extends BaseContractIntegrationTest {
    ...
    void invalidPoReference_returns400() throws Exception {
        ...
        mockMvc.perform(withAsnCreateAuth(post("/v1/inventory/asns")
    ...
    void asnCreation_producesNoGlEntry() throws Exception {
        ...
        when(asnService.createAsn(any(CreateAsnRequest.class), anyString())).thenReturn(asnResponse);
    ...
    void receiptCompleted_emitsAccrualEvent() throws Exception {
        ...
        GoodsReceiptResponse receiptResponse =
                buildGoodsReceiptResponse(receiptId, poId, locationId, expectedAccruedAmount);
    ...
    void grniBalance_availableForApMatching() throws Exception {
        ...
        when(asnService.getGoodsReceipt(eq(receiptId))).thenReturn(receiptResponse);

        mockMvc.perform(withGoodsReceiptViewAuth(get("/v1/inventory/goods-receipts/{receiptId}", receiptId)))
    ...
    void partialReceipt_asnBecomesPartiallyReceived() throws Exception {
        ...
        when(asnService.getAsn(eq(asnId))).thenReturn(asnAfterPartialReceipt);

        mockMvc.perform(withAsnViewAuth(get("/v1/inventory/asns/{asnId}", asnId)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.asnId").value(asnId.toString()))
    ...
    private MockHttpServletRequestBuilder withAsnCreateAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "asn-test-user")
                .header("X-Authorities", String.join(",", "inventory:asn:create", "inventory:asn:view"));
    }

    private MockHttpServletRequestBuilder withAsnViewAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "asn-test-user").header("X-Authorities", "inventory:asn:view");
    }
    ...
    private MockHttpServletRequestBuilder withGoodsReceiptCreateAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "asn-test-user")
                .header(
                        "X-Authorities",
                        String.join(",", "inventory:goods_receipt:create", "inventory:goods_receipt:view"));
    }

    private MockHttpServletRequestBuilder withGoodsReceiptViewAuth(MockHttpServletRequestBuilder builder) {
        return builder.header("X-User", "asn-test-user").header("X-Authorities", "inventory:goods_receipt:view");
    }
    ...
    private AsnResponse buildAsnResponse(UUID asnId, UUID vendorId, AsnStatus status) {
        return AsnResponse.builder()
                ...
                .lineItems(List.of(AsnLineResponse.builder()
                        .asnLineId(UUID.fromString("00000000-0000-0000-0000-000000000001"))
                        .sku("SKU-TEST-001")
    ...
    private GoodsReceiptResponse buildGoodsReceiptResponse(
            UUID receiptId, UUID poId, UUID locationId, long totalAccruedAmountMinor) {
        return GoodsReceiptResponse.builder()
                ...
                .createdBy("asn-test-user")
                .createdAt(Instant.now(TEST_CLOCK))
                .lines(List.of(GoodsReceiptLineResponse.builder()
                        .receiptLineId(UUID.fromString("00000000-0000-0000-0000-000000000001"))
                        .sku("SKU-TEST-001")
